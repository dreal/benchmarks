(set-logic QF_NRA)
(set-info :source |Benchmarks generated from hycomp (https://es-static.fbk.eu/tools/hycomp/). BMC instances of non-linear hybrid automata taken from: Alessandro Cimatti, Sergio Mover, Stefano Tonetta, A quantifier-free SMT encoding of non-linear hybrid automata, FMCAD 2012 and Alessandro Cimatti, Sergio Mover, Stefano Tonetta, Quantier-free encoding of invariants for Hybrid Systems, Formal Methods in System Design. This instance solves a BMC problem of depth 5 and uses the encoding obtained with quantifier elimination using redlog encoding. Contacts: Sergio Mover (mover@fbk.eu), Stefano Tonetta (tonettas@fbk.eu), Alessandro Cimatti (cimatti@fbk.eu).|)
(set-info :smt-lib-version 2.0)
(set-info :category "industrial")
(set-info :status unsat)
;; MathSAT API call trace
;; generated on Mon Mar 19 10:38:49 2012
(declare-fun b.counter.1__AT3 () Bool)
(declare-fun b.EVENT.1__AT2 () Bool)
(declare-fun b.x__AT3 () Real)
(declare-fun b.counter.2__AT3 () Bool)
(declare-fun b.y__AT0 () Real)
(declare-fun b.EVENT.1__AT3 () Bool)
(declare-fun b.counter.3__AT3 () Bool)
(declare-fun b.y__AT5 () Real)
(declare-fun b.counter.0__AT0 () Bool)
(declare-fun b.time__AT5 () Real)
(declare-fun b.bool_atom__AT3 () Bool)
(declare-fun b.counter.0__AT5 () Bool)
(declare-fun b.EVENT.0__AT0 () Bool)
(declare-fun b.counter.3__AT0 () Bool)
(declare-fun b.event_is_timed__AT4 () Bool)
(declare-fun b.counter.1__AT0 () Bool)
(declare-fun b.speed_y__AT2 () Real)
(declare-fun b.delta__AT5 () Real)
(declare-fun b.bool_atom__AT1 () Bool)
(declare-fun b.counter.1__AT5 () Bool)
(declare-fun b.x__AT1 () Real)
(declare-fun b.x__AT5 () Real)
(declare-fun b.speed_y__AT3 () Real)
(declare-fun b.EVENT.0__AT5 () Bool)
(declare-fun b.counter.2__AT5 () Bool)
(declare-fun b.EVENT.1__AT5 () Bool)
(declare-fun b.y__AT4 () Real)
(declare-fun b.time__AT4 () Real)
(declare-fun b.counter.0__AT4 () Bool)
(declare-fun b.g__AT0 () Real)
(declare-fun b.delta__AT4 () Real)
(declare-fun b.speed_y__AT0 () Real)
(declare-fun b.counter.1__AT4 () Bool)
(declare-fun b.event_is_timed__AT0 () Bool)
(declare-fun b.speed_y__AT5 () Real)
(declare-fun b.x__AT4 () Real)
(declare-fun b.EVENT.0__AT4 () Bool)
(declare-fun b.counter.2__AT4 () Bool)
(declare-fun b.y__AT1 () Real)
(declare-fun b.counter.2__AT0 () Bool)
(declare-fun b.EVENT.1__AT4 () Bool)
(declare-fun b.counter.3__AT4 () Bool)
(declare-fun b.counter.0__AT1 () Bool)
(declare-fun b.bool_atom__AT4 () Bool)
(declare-fun b.counter.1__AT1 () Bool)
(declare-fun b.counter.2__AT1 () Bool)
(declare-fun b.speed_y__AT4 () Real)
(declare-fun b.time__AT3 () Real)
(declare-fun b.counter.3__AT1 () Bool)
(declare-fun b.bool_atom__AT0 () Bool)
(declare-fun b.delta__AT3 () Real)
(declare-fun b.x__AT0 () Real)
(declare-fun b.counter.3__AT5 () Bool)
(declare-fun b.event_is_timed__AT2 () Bool)
(declare-fun b.EVENT.0__AT3 () Bool)
(declare-fun b.EVENT.1__AT0 () Bool)
(declare-fun b.bool_atom__AT5 () Bool)
(declare-fun b.counter.0__AT2 () Bool)
(declare-fun b.delta__AT1 () Real)
(declare-fun b.event_is_timed__AT3 () Bool)
(declare-fun b.counter.1__AT2 () Bool)
(declare-fun b.speed_y__AT1 () Real)
(declare-fun b.time__AT1 () Real)
(declare-fun b.EVENT.0__AT1 () Bool)
(declare-fun b.counter.2__AT2 () Bool)
(declare-fun b.bool_atom__AT2 () Bool)
(declare-fun b.time__AT0 () Real)
(declare-fun b.y__AT2 () Real)
(declare-fun b.EVENT.1__AT1 () Bool)
(declare-fun b.counter.3__AT2 () Bool)
(declare-fun b.time__AT2 () Real)
(declare-fun b.delta__AT0 () Real)
(declare-fun b.y__AT3 () Real)
(declare-fun b.delta__AT2 () Real)
(declare-fun b.counter.0__AT3 () Bool)
(declare-fun b.event_is_timed__AT1 () Bool)
(declare-fun b.event_is_timed__AT5 () Bool)
(declare-fun b.x__AT2 () Real)
(declare-fun b.EVENT.0__AT2 () Bool)
(assert (let ((.def_1161 (* b.x__AT5 b.x__AT5)))
(let ((.def_1157 (* 5.0 b.delta__AT5)))
(let ((.def_1158 (* b.delta__AT5 .def_1157)))
(let ((.def_1159 (* (- 1.0) .def_1158)))
(let ((.def_1164 (+ .def_1159 .def_1161)))
(let ((.def_1156 (* b.speed_y__AT5 b.delta__AT5)))
(let ((.def_1165 (+ .def_1156 .def_1164)))
(let ((.def_1166 (+ b.y__AT5 .def_1165)))
(let ((.def_1167 (<= 0.0 .def_1166)))
(let ((.def_1197 (not .def_1167)))
(let ((.def_1179 (* (- 10.0) b.delta__AT5)))
(let ((.def_1180 (+ b.speed_y__AT5 .def_1179)))
(let ((.def_1181 (<= 0.0 .def_1180)))
(let ((.def_1198 (and .def_1181 .def_1197)))
(let ((.def_1190 (+ b.y__AT5 .def_1161)))
(let ((.def_1195 (<= 0.0 .def_1190)))
(let ((.def_1192 (<= b.speed_y__AT5 0.0 )))
(let ((.def_1196 (or .def_1192 .def_1195)))
(let ((.def_1199 (or .def_1196 .def_1198)))
(let ((.def_1191 (<= .def_1190 0.0 )))
(let ((.def_1193 (and .def_1191 .def_1192)))
(let ((.def_1185 (<= .def_1180 0.0 )))
(let ((.def_1186 (not .def_1185)))
(let ((.def_1183 (= .def_1166 0.0 )))
(let ((.def_1184 (not .def_1183)))
(let ((.def_1187 (or .def_1184 .def_1186)))
(let ((.def_1182 (or .def_1167 .def_1181)))
(let ((.def_1188 (and .def_1182 .def_1187)))
(let ((.def_1169 (* 20.0 b.x__AT5)))
(let ((.def_1170 (* b.x__AT5 .def_1169)))
(let ((.def_1045 (* b.speed_y__AT5 b.speed_y__AT5)))
(let ((.def_1171 (+ .def_1045 .def_1170)))
(let ((.def_1172 (* 20.0 b.y__AT5)))
(let ((.def_1174 (+ .def_1172 .def_1171)))
(let ((.def_1177 (<= .def_1174 0.0 )))
(let ((.def_1178 (not .def_1177)))
(let ((.def_1189 (or .def_1178 .def_1188)))
(let ((.def_1194 (or .def_1189 .def_1193)))
(let ((.def_1200 (and .def_1194 .def_1199)))
(let ((.def_1175 (<= 0.0 .def_1174)))
(let ((.def_1176 (not .def_1175)))
(let ((.def_1201 (or .def_1176 .def_1200)))
(let ((.def_1154 (<= 0.0 b.delta__AT5)))
(let ((.def_1155 (not .def_1154)))
(let ((.def_1168 (or .def_1155 .def_1167)))
(let ((.def_1202 (and .def_1168 .def_1201)))
(let ((.def_1135 (not b.EVENT.0__AT5)))
(let ((.def_1133 (not b.EVENT.1__AT5)))
(let ((.def_1151 (and .def_1133 .def_1135)))
(let ((.def_1152 (not .def_1151)))
(let ((.def_1203 (or .def_1152 .def_1202)))
(let ((.def_846 (not b.counter.0__AT4)))
(let ((.def_836 (not b.counter.1__AT4)))
(let ((.def_1144 (and .def_836 .def_846)))
(let ((.def_1147 (or b.counter.3__AT4 .def_1144)))
(let ((.def_841 (not b.counter.2__AT4)))
(let ((.def_1145 (and .def_841 .def_1144)))
(let ((.def_855 (not b.counter.3__AT4)))
(let ((.def_1146 (or .def_855 .def_1145)))
(let ((.def_1148 (and .def_1146 .def_1147)))
(let ((.def_1138 (* (- 1.0) b.x__AT5)))
(let ((.def_1139 (* b.x__AT5 .def_1138)))
(let ((.def_1140 (<= .def_1139 b.y__AT5)))
(let ((.def_62 (<= b.g__AT0 10.0 )))
(let ((.def_61 (<= 8.0 b.g__AT0)))
(let ((.def_63 (and .def_61 .def_62)))
(let ((.def_1141 (and .def_63 .def_1140)))
(let ((.def_1136 (or .def_1133 .def_1135)))
(let ((.def_6 (not b.counter.0__AT5)))
(let ((.def_4 (not b.counter.1__AT5)))
(let ((.def_1126 (or .def_4 .def_6)))
(let ((.def_1130 (or b.counter.3__AT5 .def_1126)))
(let ((.def_1127 (or b.counter.2__AT5 .def_1126)))
(let ((.def_9 (not b.counter.2__AT5)))
(let ((.def_1125 (or .def_6 .def_9)))
(let ((.def_1128 (and .def_1125 .def_1127)))
(let ((.def_12 (not b.counter.3__AT5)))
(let ((.def_1129 (or .def_12 .def_1128)))
(let ((.def_1131 (and .def_1129 .def_1130)))
(let ((.def_1137 (and .def_1131 .def_1136)))
(let ((.def_1142 (and .def_1137 .def_1141)))
(let ((.def_946 (<= 0.0 b.delta__AT4)))
(let ((.def_927 (not b.EVENT.0__AT4)))
(let ((.def_925 (not b.EVENT.1__AT4)))
(let ((.def_943 (and .def_925 .def_927)))
(let ((.def_944 (not .def_943)))
(let ((.def_1121 (or .def_944 .def_946)))
(let ((.def_1122 (or b.EVENT.1__AT4 .def_1121)))
(let ((.def_1032 (= b.bool_atom__AT4 b.bool_atom__AT5)))
(let ((.def_1028 (= b.counter.0__AT5 b.counter.0__AT4)))
(let ((.def_1026 (= b.counter.1__AT5 b.counter.1__AT4)))
(let ((.def_1024 (= b.counter.2__AT5 b.counter.2__AT4)))
(let ((.def_1023 (= b.counter.3__AT5 b.counter.3__AT4)))
(let ((.def_1025 (and .def_1023 .def_1024)))
(let ((.def_1027 (and .def_1025 .def_1026)))
(let ((.def_1029 (and .def_1027 .def_1028)))
(let ((.def_1117 (and .def_1029 .def_1032)))
(let ((.def_1118 (or .def_944 .def_1117)))
(let ((.def_1119 (or b.EVENT.1__AT4 .def_1118)))
(let ((.def_237 (* (- 1.0) b.g__AT0)))
(let ((.def_1105 (* .def_237 b.delta__AT4)))
(let ((.def_1106 (* (- 1.0) b.speed_y__AT5)))
(let ((.def_1109 (+ .def_1106 .def_1105)))
(let ((.def_1110 (+ b.speed_y__AT4 .def_1109)))
(let ((.def_1111 (= .def_1110 0.0 )))
(let ((.def_1095 (* b.delta__AT4 b.delta__AT4)))
(let ((.def_240 (* (/ 1 2) .def_237)))
(let ((.def_1096 (* .def_240 .def_1095)))
(let ((.def_1097 (* (- 1.0) b.y__AT5)))
(let ((.def_1101 (+ .def_1097 .def_1096)))
(let ((.def_948 (* b.speed_y__AT4 b.delta__AT4)))
(let ((.def_1102 (+ .def_948 .def_1101)))
(let ((.def_1103 (+ b.y__AT4 .def_1102)))
(let ((.def_1104 (= .def_1103 0.0 )))
(let ((.def_1112 (and .def_1104 .def_1111)))
(let ((.def_1015 (= b.x__AT4 b.x__AT5)))
(let ((.def_1113 (and .def_1015 .def_1112)))
(let ((.def_1114 (or .def_944 .def_1113)))
(let ((.def_1018 (= b.y__AT4 b.y__AT5)))
(let ((.def_1090 (and .def_1015 .def_1018)))
(let ((.def_1021 (= b.speed_y__AT4 b.speed_y__AT5)))
(let ((.def_1091 (and .def_1021 .def_1090)))
(let ((.def_1087 (= b.delta__AT4 0.0 )))
(let ((.def_1088 (and .def_943 .def_1087)))
(let ((.def_1089 (not .def_1088)))
(let ((.def_1092 (or .def_1089 .def_1091)))
(let ((.def_1093 (or b.EVENT.1__AT4 .def_1092)))
(let ((.def_1080 (and .def_1021 .def_1029)))
(let ((.def_1081 (or b.bool_atom__AT4 .def_1080)))
(let ((.def_1056 (or .def_6 b.counter.0__AT4)))
(let ((.def_1055 (or b.counter.0__AT5 .def_846)))
(let ((.def_1057 (and .def_1055 .def_1056)))
(let ((.def_1058 (and .def_4 .def_1057)))
(let ((.def_1059 (or b.counter.1__AT4 .def_1058)))
(let ((.def_1054 (or b.counter.1__AT5 .def_836)))
(let ((.def_1060 (and .def_1054 .def_1059)))
(let ((.def_1071 (and .def_9 .def_1060)))
(let ((.def_1072 (or b.counter.2__AT4 .def_1071)))
(let ((.def_1066 (and .def_4 .def_846)))
(let ((.def_1067 (or b.counter.1__AT4 .def_1066)))
(let ((.def_1068 (and .def_1054 .def_1067)))
(let ((.def_1069 (and b.counter.2__AT5 .def_1068)))
(let ((.def_1070 (or .def_841 .def_1069)))
(let ((.def_1073 (and .def_1070 .def_1072)))
(let ((.def_1074 (and b.counter.3__AT5 .def_1073)))
(let ((.def_1075 (or b.counter.3__AT4 .def_1074)))
(let ((.def_1061 (and b.counter.2__AT5 .def_1060)))
(let ((.def_1062 (or b.counter.2__AT4 .def_1061)))
(let ((.def_1050 (or b.counter.1__AT5 b.counter.1__AT4)))
(let ((.def_1048 (and .def_4 b.counter.0__AT5)))
(let ((.def_1049 (or .def_836 .def_1048)))
(let ((.def_1051 (and .def_1049 .def_1050)))
(let ((.def_1052 (and .def_9 .def_1051)))
(let ((.def_1053 (or .def_841 .def_1052)))
(let ((.def_1063 (and .def_1053 .def_1062)))
(let ((.def_1064 (and .def_12 .def_1063)))
(let ((.def_1065 (or .def_855 .def_1064)))
(let ((.def_1076 (and .def_1065 .def_1075)))
(let ((.def_833 (* b.speed_y__AT4 b.speed_y__AT4)))
(let ((.def_1046 (= .def_833 .def_1045)))
(let ((.def_1043 (+ b.speed_y__AT4 b.speed_y__AT5)))
(let ((.def_1044 (= .def_1043 0.0 )))
(let ((.def_1047 (and .def_1044 .def_1046)))
(let ((.def_1077 (and .def_1047 .def_1076)))
(let ((.def_1042 (not b.bool_atom__AT4)))
(let ((.def_1078 (or .def_1042 .def_1077)))
(let ((.def_1038 (<= 0.0 b.speed_y__AT4)))
(let ((.def_1039 (not .def_1038)))
(let ((.def_930 (* (- 1.0) b.x__AT4)))
(let ((.def_931 (* b.x__AT4 .def_930)))
(let ((.def_1037 (= b.y__AT4 .def_931)))
(let ((.def_1040 (and .def_1037 .def_1039)))
(let ((.def_1041 (= b.bool_atom__AT4 .def_1040)))
(let ((.def_1079 (and .def_1041 .def_1078)))
(let ((.def_1082 (and .def_1079 .def_1081)))
(let ((.def_1083 (and .def_1015 .def_1082)))
(let ((.def_1084 (and .def_1018 .def_1083)))
(let ((.def_1085 (or .def_943 .def_1084)))
(let ((.def_1086 (or b.EVENT.1__AT4 .def_1085)))
(let ((.def_1094 (and .def_1086 .def_1093)))
(let ((.def_1115 (and .def_1094 .def_1114)))
(let ((.def_1002 (= b.time__AT4 b.time__AT5)))
(let ((.def_1016 (and .def_1002 .def_1015)))
(let ((.def_1019 (and .def_1016 .def_1018)))
(let ((.def_1022 (and .def_1019 .def_1021)))
(let ((.def_1030 (and .def_1022 .def_1029)))
(let ((.def_1033 (and .def_1030 .def_1032)))
(let ((.def_1034 (or .def_925 .def_1033)))
(let ((.def_1005 (* (- 1.0) b.time__AT5)))
(let ((.def_1008 (+ b.delta__AT4 .def_1005)))
(let ((.def_1009 (+ b.time__AT4 .def_1008)))
(let ((.def_1010 (= .def_1009 0.0 )))
(let ((.def_1011 (or .def_944 .def_1010)))
(let ((.def_1012 (or b.EVENT.1__AT4 .def_1011)))
(let ((.def_1003 (or .def_943 .def_1002)))
(let ((.def_1004 (or b.EVENT.1__AT4 .def_1003)))
(let ((.def_1013 (and .def_1004 .def_1012)))
(let ((.def_1035 (and .def_1013 .def_1034)))
(let ((.def_999 (= .def_944 b.event_is_timed__AT5)))
(let ((.def_997 (= b.event_is_timed__AT4 .def_943)))
(let ((.def_1000 (and .def_997 .def_999)))
(let ((.def_1036 (and .def_1000 .def_1035)))
(let ((.def_1116 (and .def_1036 .def_1115)))
(let ((.def_1120 (and .def_1116 .def_1119)))
(let ((.def_1123 (and .def_1120 .def_1122)))
(let ((.def_1124 (and .def_925 .def_1123)))
(let ((.def_1143 (and .def_1124 .def_1142)))
(let ((.def_1149 (and .def_1143 .def_1148)))
(let ((.def_953 (* b.x__AT4 b.x__AT4)))
(let ((.def_949 (* 5.0 b.delta__AT4)))
(let ((.def_950 (* b.delta__AT4 .def_949)))
(let ((.def_951 (* (- 1.0) .def_950)))
(let ((.def_956 (+ .def_951 .def_953)))
(let ((.def_957 (+ .def_948 .def_956)))
(let ((.def_958 (+ b.y__AT4 .def_957)))
(let ((.def_959 (<= 0.0 .def_958)))
(let ((.def_989 (not .def_959)))
(let ((.def_971 (* (- 10.0) b.delta__AT4)))
(let ((.def_972 (+ b.speed_y__AT4 .def_971)))
(let ((.def_973 (<= 0.0 .def_972)))
(let ((.def_990 (and .def_973 .def_989)))
(let ((.def_982 (+ b.y__AT4 .def_953)))
(let ((.def_987 (<= 0.0 .def_982)))
(let ((.def_984 (<= b.speed_y__AT4 0.0 )))
(let ((.def_988 (or .def_984 .def_987)))
(let ((.def_991 (or .def_988 .def_990)))
(let ((.def_983 (<= .def_982 0.0 )))
(let ((.def_985 (and .def_983 .def_984)))
(let ((.def_977 (<= .def_972 0.0 )))
(let ((.def_978 (not .def_977)))
(let ((.def_975 (= .def_958 0.0 )))
(let ((.def_976 (not .def_975)))
(let ((.def_979 (or .def_976 .def_978)))
(let ((.def_974 (or .def_959 .def_973)))
(let ((.def_980 (and .def_974 .def_979)))
(let ((.def_961 (* 20.0 b.x__AT4)))
(let ((.def_962 (* b.x__AT4 .def_961)))
(let ((.def_963 (+ .def_833 .def_962)))
(let ((.def_964 (* 20.0 b.y__AT4)))
(let ((.def_966 (+ .def_964 .def_963)))
(let ((.def_969 (<= .def_966 0.0 )))
(let ((.def_970 (not .def_969)))
(let ((.def_981 (or .def_970 .def_980)))
(let ((.def_986 (or .def_981 .def_985)))
(let ((.def_992 (and .def_986 .def_991)))
(let ((.def_967 (<= 0.0 .def_966)))
(let ((.def_968 (not .def_967)))
(let ((.def_993 (or .def_968 .def_992)))
(let ((.def_947 (not .def_946)))
(let ((.def_960 (or .def_947 .def_959)))
(let ((.def_994 (and .def_960 .def_993)))
(let ((.def_995 (or .def_944 .def_994)))
(let ((.def_630 (not b.counter.0__AT3)))
(let ((.def_620 (not b.counter.1__AT3)))
(let ((.def_936 (and .def_620 .def_630)))
(let ((.def_939 (or b.counter.3__AT3 .def_936)))
(let ((.def_625 (not b.counter.2__AT3)))
(let ((.def_937 (and .def_625 .def_936)))
(let ((.def_639 (not b.counter.3__AT3)))
(let ((.def_938 (or .def_639 .def_937)))
(let ((.def_940 (and .def_938 .def_939)))
(let ((.def_932 (<= .def_931 b.y__AT4)))
(let ((.def_933 (and .def_63 .def_932)))
(let ((.def_928 (or .def_925 .def_927)))
(let ((.def_918 (or .def_836 .def_846)))
(let ((.def_922 (or b.counter.3__AT4 .def_918)))
(let ((.def_919 (or b.counter.2__AT4 .def_918)))
(let ((.def_917 (or .def_841 .def_846)))
(let ((.def_920 (and .def_917 .def_919)))
(let ((.def_921 (or .def_855 .def_920)))
(let ((.def_923 (and .def_921 .def_922)))
(let ((.def_929 (and .def_923 .def_928)))
(let ((.def_934 (and .def_929 .def_933)))
(let ((.def_730 (<= 0.0 b.delta__AT3)))
(let ((.def_711 (not b.EVENT.0__AT3)))
(let ((.def_709 (not b.EVENT.1__AT3)))
(let ((.def_727 (and .def_709 .def_711)))
(let ((.def_728 (not .def_727)))
(let ((.def_913 (or .def_728 .def_730)))
(let ((.def_914 (or b.EVENT.1__AT3 .def_913)))
(let ((.def_820 (= b.bool_atom__AT3 b.bool_atom__AT4)))
(let ((.def_816 (= b.counter.0__AT3 b.counter.0__AT4)))
(let ((.def_813 (= b.counter.1__AT3 b.counter.1__AT4)))
(let ((.def_810 (= b.counter.2__AT3 b.counter.2__AT4)))
(let ((.def_808 (= b.counter.3__AT3 b.counter.3__AT4)))
(let ((.def_811 (and .def_808 .def_810)))
(let ((.def_814 (and .def_811 .def_813)))
(let ((.def_817 (and .def_814 .def_816)))
(let ((.def_909 (and .def_817 .def_820)))
(let ((.def_910 (or .def_728 .def_909)))
(let ((.def_911 (or b.EVENT.1__AT3 .def_910)))
(let ((.def_897 (* .def_237 b.delta__AT3)))
(let ((.def_898 (* (- 1.0) b.speed_y__AT4)))
(let ((.def_901 (+ .def_898 .def_897)))
(let ((.def_902 (+ b.speed_y__AT3 .def_901)))
(let ((.def_903 (= .def_902 0.0 )))
(let ((.def_887 (* b.delta__AT3 b.delta__AT3)))
(let ((.def_888 (* .def_240 .def_887)))
(let ((.def_889 (* (- 1.0) b.y__AT4)))
(let ((.def_893 (+ .def_889 .def_888)))
(let ((.def_732 (* b.speed_y__AT3 b.delta__AT3)))
(let ((.def_894 (+ .def_732 .def_893)))
(let ((.def_895 (+ b.y__AT3 .def_894)))
(let ((.def_896 (= .def_895 0.0 )))
(let ((.def_904 (and .def_896 .def_903)))
(let ((.def_799 (= b.x__AT3 b.x__AT4)))
(let ((.def_905 (and .def_799 .def_904)))
(let ((.def_906 (or .def_728 .def_905)))
(let ((.def_802 (= b.y__AT3 b.y__AT4)))
(let ((.def_882 (and .def_799 .def_802)))
(let ((.def_805 (= b.speed_y__AT3 b.speed_y__AT4)))
(let ((.def_883 (and .def_805 .def_882)))
(let ((.def_879 (= b.delta__AT3 0.0 )))
(let ((.def_880 (and .def_727 .def_879)))
(let ((.def_881 (not .def_880)))
(let ((.def_884 (or .def_881 .def_883)))
(let ((.def_885 (or b.EVENT.1__AT3 .def_884)))
(let ((.def_872 (and .def_805 .def_817)))
(let ((.def_873 (or b.bool_atom__AT3 .def_872)))
(let ((.def_847 (or b.counter.0__AT3 .def_846)))
(let ((.def_845 (or .def_630 b.counter.0__AT4)))
(let ((.def_848 (and .def_845 .def_847)))
(let ((.def_849 (and .def_836 .def_848)))
(let ((.def_850 (or b.counter.1__AT3 .def_849)))
(let ((.def_844 (or .def_620 b.counter.1__AT4)))
(let ((.def_851 (and .def_844 .def_850)))
(let ((.def_863 (and .def_841 .def_851)))
(let ((.def_864 (or b.counter.2__AT3 .def_863)))
(let ((.def_858 (and .def_630 .def_836)))
(let ((.def_859 (or b.counter.1__AT3 .def_858)))
(let ((.def_860 (and .def_844 .def_859)))
(let ((.def_861 (and b.counter.2__AT4 .def_860)))
(let ((.def_862 (or .def_625 .def_861)))
(let ((.def_865 (and .def_862 .def_864)))
(let ((.def_866 (and b.counter.3__AT4 .def_865)))
(let ((.def_867 (or b.counter.3__AT3 .def_866)))
(let ((.def_852 (and b.counter.2__AT4 .def_851)))
(let ((.def_853 (or b.counter.2__AT3 .def_852)))
(let ((.def_839 (or b.counter.1__AT3 b.counter.1__AT4)))
(let ((.def_837 (and b.counter.0__AT4 .def_836)))
(let ((.def_838 (or .def_620 .def_837)))
(let ((.def_840 (and .def_838 .def_839)))
(let ((.def_842 (and .def_840 .def_841)))
(let ((.def_843 (or .def_625 .def_842)))
(let ((.def_854 (and .def_843 .def_853)))
(let ((.def_856 (and .def_854 .def_855)))
(let ((.def_857 (or .def_639 .def_856)))
(let ((.def_868 (and .def_857 .def_867)))
(let ((.def_617 (* b.speed_y__AT3 b.speed_y__AT3)))
(let ((.def_834 (= .def_617 .def_833)))
(let ((.def_831 (+ b.speed_y__AT3 b.speed_y__AT4)))
(let ((.def_832 (= .def_831 0.0 )))
(let ((.def_835 (and .def_832 .def_834)))
(let ((.def_869 (and .def_835 .def_868)))
(let ((.def_830 (not b.bool_atom__AT3)))
(let ((.def_870 (or .def_830 .def_869)))
(let ((.def_826 (<= 0.0 b.speed_y__AT3)))
(let ((.def_827 (not .def_826)))
(let ((.def_714 (* (- 1.0) b.x__AT3)))
(let ((.def_715 (* b.x__AT3 .def_714)))
(let ((.def_825 (= b.y__AT3 .def_715)))
(let ((.def_828 (and .def_825 .def_827)))
(let ((.def_829 (= b.bool_atom__AT3 .def_828)))
(let ((.def_871 (and .def_829 .def_870)))
(let ((.def_874 (and .def_871 .def_873)))
(let ((.def_875 (and .def_799 .def_874)))
(let ((.def_876 (and .def_802 .def_875)))
(let ((.def_877 (or .def_727 .def_876)))
(let ((.def_878 (or b.EVENT.1__AT3 .def_877)))
(let ((.def_886 (and .def_878 .def_885)))
(let ((.def_907 (and .def_886 .def_906)))
(let ((.def_786 (= b.time__AT3 b.time__AT4)))
(let ((.def_800 (and .def_786 .def_799)))
(let ((.def_803 (and .def_800 .def_802)))
(let ((.def_806 (and .def_803 .def_805)))
(let ((.def_818 (and .def_806 .def_817)))
(let ((.def_821 (and .def_818 .def_820)))
(let ((.def_822 (or .def_709 .def_821)))
(let ((.def_789 (* (- 1.0) b.time__AT4)))
(let ((.def_792 (+ b.delta__AT3 .def_789)))
(let ((.def_793 (+ b.time__AT3 .def_792)))
(let ((.def_794 (= .def_793 0.0 )))
(let ((.def_795 (or .def_728 .def_794)))
(let ((.def_796 (or b.EVENT.1__AT3 .def_795)))
(let ((.def_787 (or .def_727 .def_786)))
(let ((.def_788 (or b.EVENT.1__AT3 .def_787)))
(let ((.def_797 (and .def_788 .def_796)))
(let ((.def_823 (and .def_797 .def_822)))
(let ((.def_783 (= .def_728 b.event_is_timed__AT4)))
(let ((.def_781 (= b.event_is_timed__AT3 .def_727)))
(let ((.def_784 (and .def_781 .def_783)))
(let ((.def_824 (and .def_784 .def_823)))
(let ((.def_908 (and .def_824 .def_907)))
(let ((.def_912 (and .def_908 .def_911)))
(let ((.def_915 (and .def_912 .def_914)))
(let ((.def_916 (and .def_709 .def_915)))
(let ((.def_935 (and .def_916 .def_934)))
(let ((.def_941 (and .def_935 .def_940)))
(let ((.def_737 (* b.x__AT3 b.x__AT3)))
(let ((.def_733 (* 5.0 b.delta__AT3)))
(let ((.def_734 (* b.delta__AT3 .def_733)))
(let ((.def_735 (* (- 1.0) .def_734)))
(let ((.def_740 (+ .def_735 .def_737)))
(let ((.def_741 (+ .def_732 .def_740)))
(let ((.def_742 (+ b.y__AT3 .def_741)))
(let ((.def_743 (<= 0.0 .def_742)))
(let ((.def_773 (not .def_743)))
(let ((.def_755 (* (- 10.0) b.delta__AT3)))
(let ((.def_756 (+ b.speed_y__AT3 .def_755)))
(let ((.def_757 (<= 0.0 .def_756)))
(let ((.def_774 (and .def_757 .def_773)))
(let ((.def_766 (+ b.y__AT3 .def_737)))
(let ((.def_771 (<= 0.0 .def_766)))
(let ((.def_768 (<= b.speed_y__AT3 0.0 )))
(let ((.def_772 (or .def_768 .def_771)))
(let ((.def_775 (or .def_772 .def_774)))
(let ((.def_767 (<= .def_766 0.0 )))
(let ((.def_769 (and .def_767 .def_768)))
(let ((.def_761 (<= .def_756 0.0 )))
(let ((.def_762 (not .def_761)))
(let ((.def_759 (= .def_742 0.0 )))
(let ((.def_760 (not .def_759)))
(let ((.def_763 (or .def_760 .def_762)))
(let ((.def_758 (or .def_743 .def_757)))
(let ((.def_764 (and .def_758 .def_763)))
(let ((.def_745 (* 20.0 b.x__AT3)))
(let ((.def_746 (* b.x__AT3 .def_745)))
(let ((.def_747 (+ .def_617 .def_746)))
(let ((.def_748 (* 20.0 b.y__AT3)))
(let ((.def_750 (+ .def_748 .def_747)))
(let ((.def_753 (<= .def_750 0.0 )))
(let ((.def_754 (not .def_753)))
(let ((.def_765 (or .def_754 .def_764)))
(let ((.def_770 (or .def_765 .def_769)))
(let ((.def_776 (and .def_770 .def_775)))
(let ((.def_751 (<= 0.0 .def_750)))
(let ((.def_752 (not .def_751)))
(let ((.def_777 (or .def_752 .def_776)))
(let ((.def_731 (not .def_730)))
(let ((.def_744 (or .def_731 .def_743)))
(let ((.def_778 (and .def_744 .def_777)))
(let ((.def_779 (or .def_728 .def_778)))
(let ((.def_414 (not b.counter.0__AT2)))
(let ((.def_404 (not b.counter.1__AT2)))
(let ((.def_720 (and .def_404 .def_414)))
(let ((.def_723 (or b.counter.3__AT2 .def_720)))
(let ((.def_409 (not b.counter.2__AT2)))
(let ((.def_721 (and .def_409 .def_720)))
(let ((.def_423 (not b.counter.3__AT2)))
(let ((.def_722 (or .def_423 .def_721)))
(let ((.def_724 (and .def_722 .def_723)))
(let ((.def_716 (<= .def_715 b.y__AT3)))
(let ((.def_717 (and .def_63 .def_716)))
(let ((.def_712 (or .def_709 .def_711)))
(let ((.def_702 (or .def_620 .def_630)))
(let ((.def_706 (or b.counter.3__AT3 .def_702)))
(let ((.def_703 (or b.counter.2__AT3 .def_702)))
(let ((.def_701 (or .def_625 .def_630)))
(let ((.def_704 (and .def_701 .def_703)))
(let ((.def_705 (or .def_639 .def_704)))
(let ((.def_707 (and .def_705 .def_706)))
(let ((.def_713 (and .def_707 .def_712)))
(let ((.def_718 (and .def_713 .def_717)))
(let ((.def_514 (<= 0.0 b.delta__AT2)))
(let ((.def_495 (not b.EVENT.0__AT2)))
(let ((.def_493 (not b.EVENT.1__AT2)))
(let ((.def_511 (and .def_493 .def_495)))
(let ((.def_512 (not .def_511)))
(let ((.def_697 (or .def_512 .def_514)))
(let ((.def_698 (or b.EVENT.1__AT2 .def_697)))
(let ((.def_604 (= b.bool_atom__AT2 b.bool_atom__AT3)))
(let ((.def_600 (= b.counter.0__AT2 b.counter.0__AT3)))
(let ((.def_597 (= b.counter.1__AT2 b.counter.1__AT3)))
(let ((.def_594 (= b.counter.2__AT2 b.counter.2__AT3)))
(let ((.def_592 (= b.counter.3__AT2 b.counter.3__AT3)))
(let ((.def_595 (and .def_592 .def_594)))
(let ((.def_598 (and .def_595 .def_597)))
(let ((.def_601 (and .def_598 .def_600)))
(let ((.def_693 (and .def_601 .def_604)))
(let ((.def_694 (or .def_512 .def_693)))
(let ((.def_695 (or b.EVENT.1__AT2 .def_694)))
(let ((.def_681 (* .def_237 b.delta__AT2)))
(let ((.def_682 (* (- 1.0) b.speed_y__AT3)))
(let ((.def_685 (+ .def_682 .def_681)))
(let ((.def_686 (+ b.speed_y__AT2 .def_685)))
(let ((.def_687 (= .def_686 0.0 )))
(let ((.def_671 (* b.delta__AT2 b.delta__AT2)))
(let ((.def_672 (* .def_240 .def_671)))
(let ((.def_673 (* (- 1.0) b.y__AT3)))
(let ((.def_677 (+ .def_673 .def_672)))
(let ((.def_516 (* b.speed_y__AT2 b.delta__AT2)))
(let ((.def_678 (+ .def_516 .def_677)))
(let ((.def_679 (+ b.y__AT2 .def_678)))
(let ((.def_680 (= .def_679 0.0 )))
(let ((.def_688 (and .def_680 .def_687)))
(let ((.def_583 (= b.x__AT2 b.x__AT3)))
(let ((.def_689 (and .def_583 .def_688)))
(let ((.def_690 (or .def_512 .def_689)))
(let ((.def_586 (= b.y__AT2 b.y__AT3)))
(let ((.def_666 (and .def_583 .def_586)))
(let ((.def_589 (= b.speed_y__AT2 b.speed_y__AT3)))
(let ((.def_667 (and .def_589 .def_666)))
(let ((.def_663 (= b.delta__AT2 0.0 )))
(let ((.def_664 (and .def_511 .def_663)))
(let ((.def_665 (not .def_664)))
(let ((.def_668 (or .def_665 .def_667)))
(let ((.def_669 (or b.EVENT.1__AT2 .def_668)))
(let ((.def_656 (and .def_589 .def_601)))
(let ((.def_657 (or b.bool_atom__AT2 .def_656)))
(let ((.def_631 (or b.counter.0__AT2 .def_630)))
(let ((.def_629 (or .def_414 b.counter.0__AT3)))
(let ((.def_632 (and .def_629 .def_631)))
(let ((.def_633 (and .def_620 .def_632)))
(let ((.def_634 (or b.counter.1__AT2 .def_633)))
(let ((.def_628 (or .def_404 b.counter.1__AT3)))
(let ((.def_635 (and .def_628 .def_634)))
(let ((.def_647 (and .def_625 .def_635)))
(let ((.def_648 (or b.counter.2__AT2 .def_647)))
(let ((.def_642 (and .def_414 .def_620)))
(let ((.def_643 (or b.counter.1__AT2 .def_642)))
(let ((.def_644 (and .def_628 .def_643)))
(let ((.def_645 (and b.counter.2__AT3 .def_644)))
(let ((.def_646 (or .def_409 .def_645)))
(let ((.def_649 (and .def_646 .def_648)))
(let ((.def_650 (and b.counter.3__AT3 .def_649)))
(let ((.def_651 (or b.counter.3__AT2 .def_650)))
(let ((.def_636 (and b.counter.2__AT3 .def_635)))
(let ((.def_637 (or b.counter.2__AT2 .def_636)))
(let ((.def_623 (or b.counter.1__AT2 b.counter.1__AT3)))
(let ((.def_621 (and b.counter.0__AT3 .def_620)))
(let ((.def_622 (or .def_404 .def_621)))
(let ((.def_624 (and .def_622 .def_623)))
(let ((.def_626 (and .def_624 .def_625)))
(let ((.def_627 (or .def_409 .def_626)))
(let ((.def_638 (and .def_627 .def_637)))
(let ((.def_640 (and .def_638 .def_639)))
(let ((.def_641 (or .def_423 .def_640)))
(let ((.def_652 (and .def_641 .def_651)))
(let ((.def_401 (* b.speed_y__AT2 b.speed_y__AT2)))
(let ((.def_618 (= .def_401 .def_617)))
(let ((.def_615 (+ b.speed_y__AT2 b.speed_y__AT3)))
(let ((.def_616 (= .def_615 0.0 )))
(let ((.def_619 (and .def_616 .def_618)))
(let ((.def_653 (and .def_619 .def_652)))
(let ((.def_614 (not b.bool_atom__AT2)))
(let ((.def_654 (or .def_614 .def_653)))
(let ((.def_610 (<= 0.0 b.speed_y__AT2)))
(let ((.def_611 (not .def_610)))
(let ((.def_498 (* (- 1.0) b.x__AT2)))
(let ((.def_499 (* b.x__AT2 .def_498)))
(let ((.def_609 (= b.y__AT2 .def_499)))
(let ((.def_612 (and .def_609 .def_611)))
(let ((.def_613 (= b.bool_atom__AT2 .def_612)))
(let ((.def_655 (and .def_613 .def_654)))
(let ((.def_658 (and .def_655 .def_657)))
(let ((.def_659 (and .def_583 .def_658)))
(let ((.def_660 (and .def_586 .def_659)))
(let ((.def_661 (or .def_511 .def_660)))
(let ((.def_662 (or b.EVENT.1__AT2 .def_661)))
(let ((.def_670 (and .def_662 .def_669)))
(let ((.def_691 (and .def_670 .def_690)))
(let ((.def_570 (= b.time__AT2 b.time__AT3)))
(let ((.def_584 (and .def_570 .def_583)))
(let ((.def_587 (and .def_584 .def_586)))
(let ((.def_590 (and .def_587 .def_589)))
(let ((.def_602 (and .def_590 .def_601)))
(let ((.def_605 (and .def_602 .def_604)))
(let ((.def_606 (or .def_493 .def_605)))
(let ((.def_573 (* (- 1.0) b.time__AT3)))
(let ((.def_576 (+ b.delta__AT2 .def_573)))
(let ((.def_577 (+ b.time__AT2 .def_576)))
(let ((.def_578 (= .def_577 0.0 )))
(let ((.def_579 (or .def_512 .def_578)))
(let ((.def_580 (or b.EVENT.1__AT2 .def_579)))
(let ((.def_571 (or .def_511 .def_570)))
(let ((.def_572 (or b.EVENT.1__AT2 .def_571)))
(let ((.def_581 (and .def_572 .def_580)))
(let ((.def_607 (and .def_581 .def_606)))
(let ((.def_567 (= .def_512 b.event_is_timed__AT3)))
(let ((.def_565 (= b.event_is_timed__AT2 .def_511)))
(let ((.def_568 (and .def_565 .def_567)))
(let ((.def_608 (and .def_568 .def_607)))
(let ((.def_692 (and .def_608 .def_691)))
(let ((.def_696 (and .def_692 .def_695)))
(let ((.def_699 (and .def_696 .def_698)))
(let ((.def_700 (and .def_493 .def_699)))
(let ((.def_719 (and .def_700 .def_718)))
(let ((.def_725 (and .def_719 .def_724)))
(let ((.def_521 (* b.x__AT2 b.x__AT2)))
(let ((.def_517 (* 5.0 b.delta__AT2)))
(let ((.def_518 (* b.delta__AT2 .def_517)))
(let ((.def_519 (* (- 1.0) .def_518)))
(let ((.def_524 (+ .def_519 .def_521)))
(let ((.def_525 (+ .def_516 .def_524)))
(let ((.def_526 (+ b.y__AT2 .def_525)))
(let ((.def_527 (<= 0.0 .def_526)))
(let ((.def_557 (not .def_527)))
(let ((.def_539 (* (- 10.0) b.delta__AT2)))
(let ((.def_540 (+ b.speed_y__AT2 .def_539)))
(let ((.def_541 (<= 0.0 .def_540)))
(let ((.def_558 (and .def_541 .def_557)))
(let ((.def_550 (+ b.y__AT2 .def_521)))
(let ((.def_555 (<= 0.0 .def_550)))
(let ((.def_552 (<= b.speed_y__AT2 0.0 )))
(let ((.def_556 (or .def_552 .def_555)))
(let ((.def_559 (or .def_556 .def_558)))
(let ((.def_551 (<= .def_550 0.0 )))
(let ((.def_553 (and .def_551 .def_552)))
(let ((.def_545 (<= .def_540 0.0 )))
(let ((.def_546 (not .def_545)))
(let ((.def_543 (= .def_526 0.0 )))
(let ((.def_544 (not .def_543)))
(let ((.def_547 (or .def_544 .def_546)))
(let ((.def_542 (or .def_527 .def_541)))
(let ((.def_548 (and .def_542 .def_547)))
(let ((.def_529 (* 20.0 b.x__AT2)))
(let ((.def_530 (* b.x__AT2 .def_529)))
(let ((.def_531 (+ .def_401 .def_530)))
(let ((.def_532 (* 20.0 b.y__AT2)))
(let ((.def_534 (+ .def_532 .def_531)))
(let ((.def_537 (<= .def_534 0.0 )))
(let ((.def_538 (not .def_537)))
(let ((.def_549 (or .def_538 .def_548)))
(let ((.def_554 (or .def_549 .def_553)))
(let ((.def_560 (and .def_554 .def_559)))
(let ((.def_535 (<= 0.0 .def_534)))
(let ((.def_536 (not .def_535)))
(let ((.def_561 (or .def_536 .def_560)))
(let ((.def_515 (not .def_514)))
(let ((.def_528 (or .def_515 .def_527)))
(let ((.def_562 (and .def_528 .def_561)))
(let ((.def_563 (or .def_512 .def_562)))
(let ((.def_196 (not b.counter.0__AT1)))
(let ((.def_186 (not b.counter.1__AT1)))
(let ((.def_504 (and .def_186 .def_196)))
(let ((.def_507 (or b.counter.3__AT1 .def_504)))
(let ((.def_191 (not b.counter.2__AT1)))
(let ((.def_505 (and .def_191 .def_504)))
(let ((.def_205 (not b.counter.3__AT1)))
(let ((.def_506 (or .def_205 .def_505)))
(let ((.def_508 (and .def_506 .def_507)))
(let ((.def_500 (<= .def_499 b.y__AT2)))
(let ((.def_501 (and .def_63 .def_500)))
(let ((.def_496 (or .def_493 .def_495)))
(let ((.def_486 (or .def_404 .def_414)))
(let ((.def_490 (or b.counter.3__AT2 .def_486)))
(let ((.def_487 (or b.counter.2__AT2 .def_486)))
(let ((.def_485 (or .def_409 .def_414)))
(let ((.def_488 (and .def_485 .def_487)))
(let ((.def_489 (or .def_423 .def_488)))
(let ((.def_491 (and .def_489 .def_490)))
(let ((.def_497 (and .def_491 .def_496)))
(let ((.def_502 (and .def_497 .def_501)))
(let ((.def_298 (<= 0.0 b.delta__AT1)))
(let ((.def_281 (not b.EVENT.0__AT1)))
(let ((.def_279 (not b.EVENT.1__AT1)))
(let ((.def_295 (and .def_279 .def_281)))
(let ((.def_296 (not .def_295)))
(let ((.def_481 (or .def_296 .def_298)))
(let ((.def_482 (or b.EVENT.1__AT1 .def_481)))
(let ((.def_388 (= b.bool_atom__AT1 b.bool_atom__AT2)))
(let ((.def_384 (= b.counter.0__AT1 b.counter.0__AT2)))
(let ((.def_381 (= b.counter.1__AT1 b.counter.1__AT2)))
(let ((.def_378 (= b.counter.2__AT1 b.counter.2__AT2)))
(let ((.def_376 (= b.counter.3__AT1 b.counter.3__AT2)))
(let ((.def_379 (and .def_376 .def_378)))
(let ((.def_382 (and .def_379 .def_381)))
(let ((.def_385 (and .def_382 .def_384)))
(let ((.def_477 (and .def_385 .def_388)))
(let ((.def_478 (or .def_296 .def_477)))
(let ((.def_479 (or b.EVENT.1__AT1 .def_478)))
(let ((.def_465 (* .def_237 b.delta__AT1)))
(let ((.def_466 (* (- 1.0) b.speed_y__AT2)))
(let ((.def_469 (+ .def_466 .def_465)))
(let ((.def_470 (+ b.speed_y__AT1 .def_469)))
(let ((.def_471 (= .def_470 0.0 )))
(let ((.def_455 (* b.delta__AT1 b.delta__AT1)))
(let ((.def_456 (* .def_240 .def_455)))
(let ((.def_457 (* (- 1.0) b.y__AT2)))
(let ((.def_461 (+ .def_457 .def_456)))
(let ((.def_300 (* b.speed_y__AT1 b.delta__AT1)))
(let ((.def_462 (+ .def_300 .def_461)))
(let ((.def_463 (+ b.y__AT1 .def_462)))
(let ((.def_464 (= .def_463 0.0 )))
(let ((.def_472 (and .def_464 .def_471)))
(let ((.def_367 (= b.x__AT1 b.x__AT2)))
(let ((.def_473 (and .def_367 .def_472)))
(let ((.def_474 (or .def_296 .def_473)))
(let ((.def_370 (= b.y__AT1 b.y__AT2)))
(let ((.def_450 (and .def_367 .def_370)))
(let ((.def_373 (= b.speed_y__AT1 b.speed_y__AT2)))
(let ((.def_451 (and .def_373 .def_450)))
(let ((.def_447 (= b.delta__AT1 0.0 )))
(let ((.def_448 (and .def_295 .def_447)))
(let ((.def_449 (not .def_448)))
(let ((.def_452 (or .def_449 .def_451)))
(let ((.def_453 (or b.EVENT.1__AT1 .def_452)))
(let ((.def_440 (and .def_373 .def_385)))
(let ((.def_441 (or b.bool_atom__AT1 .def_440)))
(let ((.def_415 (or b.counter.0__AT1 .def_414)))
(let ((.def_413 (or .def_196 b.counter.0__AT2)))
(let ((.def_416 (and .def_413 .def_415)))
(let ((.def_417 (and .def_404 .def_416)))
(let ((.def_418 (or b.counter.1__AT1 .def_417)))
(let ((.def_412 (or .def_186 b.counter.1__AT2)))
(let ((.def_419 (and .def_412 .def_418)))
(let ((.def_431 (and .def_409 .def_419)))
(let ((.def_432 (or b.counter.2__AT1 .def_431)))
(let ((.def_426 (and .def_196 .def_404)))
(let ((.def_427 (or b.counter.1__AT1 .def_426)))
(let ((.def_428 (and .def_412 .def_427)))
(let ((.def_429 (and b.counter.2__AT2 .def_428)))
(let ((.def_430 (or .def_191 .def_429)))
(let ((.def_433 (and .def_430 .def_432)))
(let ((.def_434 (and b.counter.3__AT2 .def_433)))
(let ((.def_435 (or b.counter.3__AT1 .def_434)))
(let ((.def_420 (and b.counter.2__AT2 .def_419)))
(let ((.def_421 (or b.counter.2__AT1 .def_420)))
(let ((.def_407 (or b.counter.1__AT1 b.counter.1__AT2)))
(let ((.def_405 (and b.counter.0__AT2 .def_404)))
(let ((.def_406 (or .def_186 .def_405)))
(let ((.def_408 (and .def_406 .def_407)))
(let ((.def_410 (and .def_408 .def_409)))
(let ((.def_411 (or .def_191 .def_410)))
(let ((.def_422 (and .def_411 .def_421)))
(let ((.def_424 (and .def_422 .def_423)))
(let ((.def_425 (or .def_205 .def_424)))
(let ((.def_436 (and .def_425 .def_435)))
(let ((.def_183 (* b.speed_y__AT1 b.speed_y__AT1)))
(let ((.def_402 (= .def_183 .def_401)))
(let ((.def_399 (+ b.speed_y__AT1 b.speed_y__AT2)))
(let ((.def_400 (= .def_399 0.0 )))
(let ((.def_403 (and .def_400 .def_402)))
(let ((.def_437 (and .def_403 .def_436)))
(let ((.def_398 (not b.bool_atom__AT1)))
(let ((.def_438 (or .def_398 .def_437)))
(let ((.def_394 (<= 0.0 b.speed_y__AT1)))
(let ((.def_395 (not .def_394)))
(let ((.def_284 (* (- 1.0) b.x__AT1)))
(let ((.def_285 (* b.x__AT1 .def_284)))
(let ((.def_393 (= b.y__AT1 .def_285)))
(let ((.def_396 (and .def_393 .def_395)))
(let ((.def_397 (= b.bool_atom__AT1 .def_396)))
(let ((.def_439 (and .def_397 .def_438)))
(let ((.def_442 (and .def_439 .def_441)))
(let ((.def_443 (and .def_367 .def_442)))
(let ((.def_444 (and .def_370 .def_443)))
(let ((.def_445 (or .def_295 .def_444)))
(let ((.def_446 (or b.EVENT.1__AT1 .def_445)))
(let ((.def_454 (and .def_446 .def_453)))
(let ((.def_475 (and .def_454 .def_474)))
(let ((.def_354 (= b.time__AT1 b.time__AT2)))
(let ((.def_368 (and .def_354 .def_367)))
(let ((.def_371 (and .def_368 .def_370)))
(let ((.def_374 (and .def_371 .def_373)))
(let ((.def_386 (and .def_374 .def_385)))
(let ((.def_389 (and .def_386 .def_388)))
(let ((.def_390 (or .def_279 .def_389)))
(let ((.def_357 (* (- 1.0) b.time__AT2)))
(let ((.def_360 (+ b.delta__AT1 .def_357)))
(let ((.def_361 (+ b.time__AT1 .def_360)))
(let ((.def_362 (= .def_361 0.0 )))
(let ((.def_363 (or .def_296 .def_362)))
(let ((.def_364 (or b.EVENT.1__AT1 .def_363)))
(let ((.def_355 (or .def_295 .def_354)))
(let ((.def_356 (or b.EVENT.1__AT1 .def_355)))
(let ((.def_365 (and .def_356 .def_364)))
(let ((.def_391 (and .def_365 .def_390)))
(let ((.def_351 (= .def_296 b.event_is_timed__AT2)))
(let ((.def_349 (= b.event_is_timed__AT1 .def_295)))
(let ((.def_352 (and .def_349 .def_351)))
(let ((.def_392 (and .def_352 .def_391)))
(let ((.def_476 (and .def_392 .def_475)))
(let ((.def_480 (and .def_476 .def_479)))
(let ((.def_483 (and .def_480 .def_482)))
(let ((.def_484 (and .def_279 .def_483)))
(let ((.def_503 (and .def_484 .def_502)))
(let ((.def_509 (and .def_503 .def_508)))
(let ((.def_305 (* b.x__AT1 b.x__AT1)))
(let ((.def_301 (* 5.0 b.delta__AT1)))
(let ((.def_302 (* b.delta__AT1 .def_301)))
(let ((.def_303 (* (- 1.0) .def_302)))
(let ((.def_308 (+ .def_303 .def_305)))
(let ((.def_309 (+ .def_300 .def_308)))
(let ((.def_310 (+ b.y__AT1 .def_309)))
(let ((.def_311 (<= 0.0 .def_310)))
(let ((.def_341 (not .def_311)))
(let ((.def_323 (* (- 10.0) b.delta__AT1)))
(let ((.def_324 (+ b.speed_y__AT1 .def_323)))
(let ((.def_325 (<= 0.0 .def_324)))
(let ((.def_342 (and .def_325 .def_341)))
(let ((.def_334 (+ b.y__AT1 .def_305)))
(let ((.def_339 (<= 0.0 .def_334)))
(let ((.def_336 (<= b.speed_y__AT1 0.0 )))
(let ((.def_340 (or .def_336 .def_339)))
(let ((.def_343 (or .def_340 .def_342)))
(let ((.def_335 (<= .def_334 0.0 )))
(let ((.def_337 (and .def_335 .def_336)))
(let ((.def_329 (<= .def_324 0.0 )))
(let ((.def_330 (not .def_329)))
(let ((.def_327 (= .def_310 0.0 )))
(let ((.def_328 (not .def_327)))
(let ((.def_331 (or .def_328 .def_330)))
(let ((.def_326 (or .def_311 .def_325)))
(let ((.def_332 (and .def_326 .def_331)))
(let ((.def_313 (* 20.0 b.x__AT1)))
(let ((.def_314 (* b.x__AT1 .def_313)))
(let ((.def_315 (+ .def_183 .def_314)))
(let ((.def_316 (* 20.0 b.y__AT1)))
(let ((.def_318 (+ .def_316 .def_315)))
(let ((.def_321 (<= .def_318 0.0 )))
(let ((.def_322 (not .def_321)))
(let ((.def_333 (or .def_322 .def_332)))
(let ((.def_338 (or .def_333 .def_337)))
(let ((.def_344 (and .def_338 .def_343)))
(let ((.def_319 (<= 0.0 .def_318)))
(let ((.def_320 (not .def_319)))
(let ((.def_345 (or .def_320 .def_344)))
(let ((.def_299 (not .def_298)))
(let ((.def_312 (or .def_299 .def_311)))
(let ((.def_346 (and .def_312 .def_345)))
(let ((.def_347 (or .def_296 .def_346)))
(let ((.def_32 (not b.counter.0__AT0)))
(let ((.def_30 (not b.counter.1__AT0)))
(let ((.def_33 (and .def_30 .def_32)))
(let ((.def_291 (or .def_33 b.counter.3__AT0)))
(let ((.def_38 (not b.counter.3__AT0)))
(let ((.def_35 (not b.counter.2__AT0)))
(let ((.def_36 (and .def_33 .def_35)))
(let ((.def_290 (or .def_36 .def_38)))
(let ((.def_292 (and .def_290 .def_291)))
(let ((.def_286 (<= .def_285 b.y__AT1)))
(let ((.def_287 (and .def_63 .def_286)))
(let ((.def_282 (or .def_279 .def_281)))
(let ((.def_272 (or .def_186 .def_196)))
(let ((.def_276 (or b.counter.3__AT1 .def_272)))
(let ((.def_273 (or b.counter.2__AT1 .def_272)))
(let ((.def_271 (or .def_191 .def_196)))
(let ((.def_274 (and .def_271 .def_273)))
(let ((.def_275 (or .def_205 .def_274)))
(let ((.def_277 (and .def_275 .def_276)))
(let ((.def_283 (and .def_277 .def_282)))
(let ((.def_288 (and .def_283 .def_287)))
(let ((.def_71 (<= 0.0 b.delta__AT0)))
(let ((.def_51 (not b.EVENT.0__AT0)))
(let ((.def_49 (not b.EVENT.1__AT0)))
(let ((.def_68 (and .def_49 .def_51)))
(let ((.def_69 (not .def_68)))
(let ((.def_267 (or .def_69 .def_71)))
(let ((.def_268 (or b.EVENT.1__AT0 .def_267)))
(let ((.def_170 (= b.bool_atom__AT0 b.bool_atom__AT1)))
(let ((.def_165 (= b.counter.0__AT0 b.counter.0__AT1)))
(let ((.def_162 (= b.counter.1__AT0 b.counter.1__AT1)))
(let ((.def_159 (= b.counter.2__AT0 b.counter.2__AT1)))
(let ((.def_157 (= b.counter.3__AT0 b.counter.3__AT1)))
(let ((.def_160 (and .def_157 .def_159)))
(let ((.def_163 (and .def_160 .def_162)))
(let ((.def_166 (and .def_163 .def_165)))
(let ((.def_263 (and .def_166 .def_170)))
(let ((.def_264 (or .def_69 .def_263)))
(let ((.def_265 (or b.EVENT.1__AT0 .def_264)))
(let ((.def_251 (* b.delta__AT0 .def_237)))
(let ((.def_252 (* (- 1.0) b.speed_y__AT1)))
(let ((.def_255 (+ .def_252 .def_251)))
(let ((.def_256 (+ b.speed_y__AT0 .def_255)))
(let ((.def_257 (= .def_256 0.0 )))
(let ((.def_241 (* b.delta__AT0 b.delta__AT0)))
(let ((.def_242 (* .def_240 .def_241)))
(let ((.def_244 (* (- 1.0) b.y__AT1)))
(let ((.def_247 (+ .def_244 .def_242)))
(let ((.def_74 (* b.delta__AT0 b.speed_y__AT0)))
(let ((.def_248 (+ .def_74 .def_247)))
(let ((.def_249 (+ b.y__AT0 .def_248)))
(let ((.def_250 (= .def_249 0.0 )))
(let ((.def_258 (and .def_250 .def_257)))
(let ((.def_148 (= b.x__AT0 b.x__AT1)))
(let ((.def_259 (and .def_148 .def_258)))
(let ((.def_260 (or .def_69 .def_259)))
(let ((.def_151 (= b.y__AT0 b.y__AT1)))
(let ((.def_232 (and .def_148 .def_151)))
(let ((.def_154 (= b.speed_y__AT0 b.speed_y__AT1)))
(let ((.def_233 (and .def_154 .def_232)))
(let ((.def_229 (= b.delta__AT0 0.0 )))
(let ((.def_230 (and .def_68 .def_229)))
(let ((.def_231 (not .def_230)))
(let ((.def_234 (or .def_231 .def_233)))
(let ((.def_235 (or b.EVENT.1__AT0 .def_234)))
(let ((.def_222 (and .def_154 .def_166)))
(let ((.def_223 (or b.bool_atom__AT0 .def_222)))
(let ((.def_197 (or b.counter.0__AT0 .def_196)))
(let ((.def_195 (or .def_32 b.counter.0__AT1)))
(let ((.def_198 (and .def_195 .def_197)))
(let ((.def_199 (and .def_186 .def_198)))
(let ((.def_200 (or b.counter.1__AT0 .def_199)))
(let ((.def_194 (or .def_30 b.counter.1__AT1)))
(let ((.def_201 (and .def_194 .def_200)))
(let ((.def_213 (and .def_191 .def_201)))
(let ((.def_214 (or b.counter.2__AT0 .def_213)))
(let ((.def_208 (and .def_32 .def_186)))
(let ((.def_209 (or b.counter.1__AT0 .def_208)))
(let ((.def_210 (and .def_194 .def_209)))
(let ((.def_211 (and b.counter.2__AT1 .def_210)))
(let ((.def_212 (or .def_35 .def_211)))
(let ((.def_215 (and .def_212 .def_214)))
(let ((.def_216 (and b.counter.3__AT1 .def_215)))
(let ((.def_217 (or b.counter.3__AT0 .def_216)))
(let ((.def_202 (and b.counter.2__AT1 .def_201)))
(let ((.def_203 (or b.counter.2__AT0 .def_202)))
(let ((.def_189 (or b.counter.1__AT0 b.counter.1__AT1)))
(let ((.def_187 (and b.counter.0__AT1 .def_186)))
(let ((.def_188 (or .def_30 .def_187)))
(let ((.def_190 (and .def_188 .def_189)))
(let ((.def_192 (and .def_190 .def_191)))
(let ((.def_193 (or .def_35 .def_192)))
(let ((.def_204 (and .def_193 .def_203)))
(let ((.def_206 (and .def_204 .def_205)))
(let ((.def_207 (or .def_38 .def_206)))
(let ((.def_218 (and .def_207 .def_217)))
(let ((.def_88 (* b.speed_y__AT0 b.speed_y__AT0)))
(let ((.def_184 (= .def_88 .def_183)))
(let ((.def_181 (+ b.speed_y__AT0 b.speed_y__AT1)))
(let ((.def_182 (= .def_181 0.0 )))
(let ((.def_185 (and .def_182 .def_184)))
(let ((.def_219 (and .def_185 .def_218)))
(let ((.def_180 (not b.bool_atom__AT0)))
(let ((.def_220 (or .def_180 .def_219)))
(let ((.def_176 (<= 0.0 b.speed_y__AT0)))
(let ((.def_177 (not .def_176)))
(let ((.def_56 (* (- 1.0) b.x__AT0)))
(let ((.def_57 (* b.x__AT0 .def_56)))
(let ((.def_175 (= b.y__AT0 .def_57)))
(let ((.def_178 (and .def_175 .def_177)))
(let ((.def_179 (= b.bool_atom__AT0 .def_178)))
(let ((.def_221 (and .def_179 .def_220)))
(let ((.def_224 (and .def_221 .def_223)))
(let ((.def_225 (and .def_148 .def_224)))
(let ((.def_226 (and .def_151 .def_225)))
(let ((.def_227 (or .def_68 .def_226)))
(let ((.def_228 (or b.EVENT.1__AT0 .def_227)))
(let ((.def_236 (and .def_228 .def_235)))
(let ((.def_261 (and .def_236 .def_260)))
(let ((.def_135 (= b.time__AT0 b.time__AT1)))
(let ((.def_149 (and .def_135 .def_148)))
(let ((.def_152 (and .def_149 .def_151)))
(let ((.def_155 (and .def_152 .def_154)))
(let ((.def_167 (and .def_155 .def_166)))
(let ((.def_171 (and .def_167 .def_170)))
(let ((.def_172 (or .def_49 .def_171)))
(let ((.def_138 (* (- 1.0) b.time__AT1)))
(let ((.def_141 (+ b.delta__AT0 .def_138)))
(let ((.def_142 (+ b.time__AT0 .def_141)))
(let ((.def_143 (= .def_142 0.0 )))
(let ((.def_144 (or .def_69 .def_143)))
(let ((.def_145 (or b.EVENT.1__AT0 .def_144)))
(let ((.def_136 (or .def_68 .def_135)))
(let ((.def_137 (or b.EVENT.1__AT0 .def_136)))
(let ((.def_146 (and .def_137 .def_145)))
(let ((.def_173 (and .def_146 .def_172)))
(let ((.def_132 (= .def_69 b.event_is_timed__AT1)))
(let ((.def_130 (= b.event_is_timed__AT0 .def_68)))
(let ((.def_133 (and .def_130 .def_132)))
(let ((.def_174 (and .def_133 .def_173)))
(let ((.def_262 (and .def_174 .def_261)))
(let ((.def_266 (and .def_262 .def_265)))
(let ((.def_269 (and .def_266 .def_268)))
(let ((.def_270 (and .def_49 .def_269)))
(let ((.def_289 (and .def_270 .def_288)))
(let ((.def_293 (and .def_289 .def_292)))
(let ((.def_80 (* b.x__AT0 b.x__AT0)))
(let ((.def_76 (* 5.0 b.delta__AT0)))
(let ((.def_77 (* b.delta__AT0 .def_76)))
(let ((.def_78 (* (- 1.0) .def_77)))
(let ((.def_83 (+ .def_78 .def_80)))
(let ((.def_84 (+ .def_74 .def_83)))
(let ((.def_85 (+ b.y__AT0 .def_84)))
(let ((.def_86 (<= 0.0 .def_85)))
(let ((.def_122 (not .def_86)))
(let ((.def_103 (* (- 1.0) b.speed_y__AT0)))
(let ((.def_104 (* 10.0 b.delta__AT0)))
(let ((.def_105 (+ .def_104 .def_103)))
(let ((.def_106 (<= .def_105 0.0 )))
(let ((.def_123 (and .def_106 .def_122)))
(let ((.def_115 (+ b.y__AT0 .def_80)))
(let ((.def_120 (<= 0.0 .def_115)))
(let ((.def_117 (<= b.speed_y__AT0 0.0 )))
(let ((.def_121 (or .def_117 .def_120)))
(let ((.def_124 (or .def_121 .def_123)))
(let ((.def_116 (<= .def_115 0.0 )))
(let ((.def_118 (and .def_116 .def_117)))
(let ((.def_110 (<= 0.0 .def_105)))
(let ((.def_111 (not .def_110)))
(let ((.def_108 (= .def_85 0.0 )))
(let ((.def_109 (not .def_108)))
(let ((.def_112 (or .def_109 .def_111)))
(let ((.def_107 (or .def_86 .def_106)))
(let ((.def_113 (and .def_107 .def_112)))
(let ((.def_90 (* 20.0 b.x__AT0)))
(let ((.def_91 (* b.x__AT0 .def_90)))
(let ((.def_92 (+ .def_88 .def_91)))
(let ((.def_93 (* 20.0 b.y__AT0)))
(let ((.def_95 (+ .def_93 .def_92)))
(let ((.def_98 (<= .def_95 0.0 )))
(let ((.def_99 (not .def_98)))
(let ((.def_114 (or .def_99 .def_113)))
(let ((.def_119 (or .def_114 .def_118)))
(let ((.def_125 (and .def_119 .def_124)))
(let ((.def_96 (<= 0.0 .def_95)))
(let ((.def_97 (not .def_96)))
(let ((.def_126 (or .def_97 .def_125)))
(let ((.def_72 (not .def_71)))
(let ((.def_87 (or .def_72 .def_86)))
(let ((.def_127 (and .def_87 .def_126)))
(let ((.def_128 (or .def_69 .def_127)))
(let ((.def_58 (<= .def_57 b.y__AT0)))
(let ((.def_64 (and .def_58 .def_63)))
(let ((.def_52 (or .def_49 .def_51)))
(let ((.def_42 (or .def_30 .def_32)))
(let ((.def_46 (or b.counter.3__AT0 .def_42)))
(let ((.def_43 (or b.counter.2__AT0 .def_42)))
(let ((.def_41 (or .def_32 .def_35)))
(let ((.def_44 (and .def_41 .def_43)))
(let ((.def_45 (or .def_38 .def_44)))
(let ((.def_47 (and .def_45 .def_46)))
(let ((.def_53 (and .def_47 .def_52)))
(let ((.def_65 (and .def_53 .def_64)))
(let ((.def_39 (and .def_36 .def_38)))
(let ((.def_27 (= b.y__AT0 10.0 )))
(let ((.def_23 (= b.x__AT0 0.0 )))
(let ((.def_19 (= b.time__AT0 0.0 )))
(let ((.def_21 (and .def_19 b.event_is_timed__AT0)))
(let ((.def_24 (and .def_21 .def_23)))
(let ((.def_28 (and .def_24 .def_27)))
(let ((.def_40 (and .def_28 .def_39)))
(let ((.def_66 (and .def_40 .def_65)))
(let ((.def_7 (and .def_4 .def_6)))
(let ((.def_14 (or .def_7 b.counter.3__AT5)))
(let ((.def_10 (and .def_7 .def_9)))
(let ((.def_13 (or .def_10 .def_12)))
(let ((.def_15 (and .def_13 .def_14)))
(let ((.def_16 (not .def_15)))
(let ((.def_67 (and .def_16 .def_66)))
(let ((.def_129 (and .def_67 .def_128)))
(let ((.def_294 (and .def_129 .def_293)))
(let ((.def_348 (and .def_294 .def_347)))
(let ((.def_510 (and .def_348 .def_509)))
(let ((.def_564 (and .def_510 .def_563)))
(let ((.def_726 (and .def_564 .def_725)))
(let ((.def_780 (and .def_726 .def_779)))
(let ((.def_942 (and .def_780 .def_941)))
(let ((.def_996 (and .def_942 .def_995)))
(let ((.def_1150 (and .def_996 .def_1149)))
(let ((.def_1204 (and .def_1150 .def_1203)))
.def_1204)))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))
(check-sat)
(exit)
