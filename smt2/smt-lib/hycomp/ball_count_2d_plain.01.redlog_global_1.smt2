(set-logic QF_NRA)
(set-info :source |Benchmarks generated from hycomp (https://es-static.fbk.eu/tools/hycomp/). BMC instances of non-linear hybrid automata taken from: Alessandro Cimatti, Sergio Mover, Stefano Tonetta, A quantifier-free SMT encoding of non-linear hybrid automata, FMCAD 2012 and Alessandro Cimatti, Sergio Mover, Stefano Tonetta, Quantier-free encoding of invariants for Hybrid Systems, Formal Methods in System Design. This instance solves a BMC problem of depth 1 and uses the encoding obtained with quantifier elimination using redlog encoding. Contacts: Sergio Mover (mover@fbk.eu), Stefano Tonetta (tonettas@fbk.eu), Alessandro Cimatti (cimatti@fbk.eu).|)
(set-info :smt-lib-version 2.0)
(set-info :category "industrial")
(set-info :status unsat)
;; MathSAT API call trace
;; generated on Mon Mar 19 10:42:51 2012
(declare-fun b.y__AT2 () Real)
(declare-fun b.EVENT.1__AT0 () Bool)
(declare-fun b.time__AT0 () Real)
(declare-fun b.speed_x__AT1 () Real)
(declare-fun b.speed_y__AT1 () Real)
(declare-fun b.y__AT0 () Real)
(declare-fun b.counter.0__AT0 () Bool)
(declare-fun b.counter.1__AT0 () Bool)
(declare-fun g__AT0 () Real)
(declare-fun b.counter.2__AT0 () Bool)
(declare-fun b.time__AT1 () Real)
(declare-fun b.delta__AT1 () Real)
(declare-fun b.delta__AT0 () Real)
(declare-fun b.event_is_timed__AT0 () Bool)
(declare-fun b.EVENT.0__AT0 () Bool)
(declare-fun b.y__AT1 () Real)
(declare-fun b.counter.0__AT1 () Bool)
(declare-fun b.counter.1__AT1 () Bool)
(declare-fun b.counter.2__AT1 () Bool)
(declare-fun speed_loss__AT0 () Real)
(declare-fun b.counter.3__AT1 () Bool)
(declare-fun b.EVENT.0__AT1 () Bool)
(declare-fun b.EVENT.1__AT1 () Bool)
(declare-fun b.counter.3__AT0 () Bool)
(declare-fun b.x__AT0 () Real)
(declare-fun b.speed_y__AT0 () Real)
(declare-fun b.speed_x__AT0 () Real)
(declare-fun b.x__AT1 () Real)
(declare-fun b.event_is_timed__AT1 () Bool)
(assert (let ((.def_416 (= b.y__AT1 b.y__AT2)))
(let ((.def_417 (not .def_416)))
(let ((.def_330 (* g__AT0 b.delta__AT1)))
(let ((.def_404 (<= b.speed_y__AT1 .def_330)))
(let ((.def_405 (not .def_404)))
(let ((.def_333 (* 2.0 b.speed_y__AT1)))
(let ((.def_334 (* b.delta__AT1 .def_333)))
(let ((.def_331 (* b.delta__AT1 .def_330)))
(let ((.def_332 (* (- 1.0) .def_331)))
(let ((.def_335 (+ .def_332 .def_334)))
(let ((.def_336 (* 2.0 b.y__AT1)))
(let ((.def_338 (+ .def_336 .def_335)))
(let ((.def_396 (= .def_338 0.0 )))
(let ((.def_397 (not .def_396)))
(let ((.def_406 (or .def_397 .def_405)))
(let ((.def_111 (* 2.0 g__AT0)))
(let ((.def_383 (* .def_111 b.speed_y__AT1)))
(let ((.def_384 (* b.delta__AT1 .def_383)))
(let ((.def_131 (* g__AT0 g__AT0)))
(let ((.def_380 (* .def_131 b.delta__AT1)))
(let ((.def_381 (* b.delta__AT1 .def_380)))
(let ((.def_382 (* (- 1.0) .def_381)))
(let ((.def_385 (+ .def_382 .def_384)))
(let ((.def_365 (* .def_111 b.y__AT1)))
(let ((.def_386 (+ .def_365 .def_385)))
(let ((.def_391 (<= 0.0 .def_386)))
(let ((.def_343 (* g__AT0 b.speed_y__AT1)))
(let ((.def_388 (<= .def_380 .def_343)))
(let ((.def_392 (or .def_388 .def_391)))
(let ((.def_407 (and .def_392 .def_406)))
(let ((.def_345 (* b.speed_y__AT1 b.speed_y__AT1)))
(let ((.def_366 (+ .def_345 .def_365)))
(let ((.def_401 (<= .def_366 0.0 )))
(let ((.def_402 (not .def_401)))
(let ((.def_127 (<= 0.0 g__AT0)))
(let ((.def_128 (not .def_127)))
(let ((.def_403 (or .def_128 .def_402)))
(let ((.def_408 (or .def_403 .def_407)))
(let ((.def_370 (* g__AT0 b.y__AT1)))
(let ((.def_374 (<= .def_370 0.0 )))
(let ((.def_373 (<= .def_343 0.0 )))
(let ((.def_375 (and .def_373 .def_374)))
(let ((.def_409 (or .def_375 .def_408)))
(let ((.def_394 (<= .def_330 b.speed_y__AT1)))
(let ((.def_395 (not .def_394)))
(let ((.def_398 (or .def_395 .def_397)))
(let ((.def_387 (<= .def_386 0.0 )))
(let ((.def_389 (and .def_387 .def_388)))
(let ((.def_390 (or .def_128 .def_389)))
(let ((.def_393 (and .def_390 .def_392)))
(let ((.def_399 (and .def_393 .def_398)))
(let ((.def_377 (= .def_366 0.0 )))
(let ((.def_378 (and .def_128 .def_377)))
(let ((.def_371 (<= 0.0 .def_370)))
(let ((.def_119 (<= g__AT0 0.0 )))
(let ((.def_120 (not .def_119)))
(let ((.def_372 (and .def_120 .def_371)))
(let ((.def_376 (or .def_372 .def_375)))
(let ((.def_379 (or .def_376 .def_378)))
(let ((.def_400 (or .def_379 .def_399)))
(let ((.def_410 (and .def_400 .def_409)))
(let ((.def_367 (<= 0.0 .def_366)))
(let ((.def_368 (not .def_367)))
(let ((.def_94 (= g__AT0 0.0 )))
(let ((.def_369 (or .def_94 .def_368)))
(let ((.def_411 (or .def_369 .def_410)))
(let ((.def_358 (* b.speed_y__AT1 b.delta__AT1)))
(let ((.def_359 (+ b.y__AT1 .def_358)))
(let ((.def_360 (= .def_359 0.0 )))
(let ((.def_361 (not .def_360)))
(let ((.def_355 (* b.delta__AT1 .def_345)))
(let ((.def_350 (* b.y__AT1 b.speed_y__AT1)))
(let ((.def_356 (+ .def_350 .def_355)))
(let ((.def_357 (<= .def_356 0.0 )))
(let ((.def_362 (and .def_357 .def_361)))
(let ((.def_353 (= b.speed_y__AT1 0.0 )))
(let ((.def_351 (<= 0.0 .def_350)))
(let ((.def_346 (* b.speed_y__AT1 .def_345)))
(let ((.def_344 (* b.y__AT1 .def_343)))
(let ((.def_347 (+ .def_344 .def_346)))
(let ((.def_348 (<= .def_347 0.0 )))
(let ((.def_95 (not .def_94)))
(let ((.def_349 (or .def_95 .def_348)))
(let ((.def_352 (or .def_349 .def_351)))
(let ((.def_354 (or .def_352 .def_353)))
(let ((.def_363 (or .def_354 .def_362)))
(let ((.def_340 (<= 0.0 b.delta__AT1)))
(let ((.def_341 (not .def_340)))
(let ((.def_339 (<= 0.0 .def_338)))
(let ((.def_342 (or .def_339 .def_341)))
(let ((.def_364 (and .def_342 .def_363)))
(let ((.def_412 (and .def_364 .def_411)))
(let ((.def_318 (not b.EVENT.0__AT1)))
(let ((.def_316 (not b.EVENT.1__AT1)))
(let ((.def_327 (and .def_316 .def_318)))
(let ((.def_413 (and .def_327 .def_412)))
(let ((.def_414 (not .def_413)))
(let ((.def_418 (or .def_414 .def_417)))
(let ((.def_328 (not .def_327)))
(let ((.def_419 (or .def_328 .def_418)))
(let ((.def_321 (<= 0.0 b.y__AT1)))
(let ((.def_63 (<= speed_loss__AT0 (/ 1 2))))
(let ((.def_60 (<= (/ 1 10) speed_loss__AT0)))
(let ((.def_64 (and .def_60 .def_63)))
(let ((.def_55 (<= g__AT0 10.0 )))
(let ((.def_54 (<= 8.0 g__AT0)))
(let ((.def_56 (and .def_54 .def_55)))
(let ((.def_65 (and .def_56 .def_64)))
(let ((.def_322 (and .def_65 .def_321)))
(let ((.def_319 (or .def_316 .def_318)))
(let ((.def_6 (not b.counter.0__AT1)))
(let ((.def_4 (not b.counter.1__AT1)))
(let ((.def_309 (or .def_4 .def_6)))
(let ((.def_313 (or b.counter.3__AT1 .def_309)))
(let ((.def_310 (or b.counter.2__AT1 .def_309)))
(let ((.def_9 (not b.counter.2__AT1)))
(let ((.def_308 (or .def_6 .def_9)))
(let ((.def_311 (and .def_308 .def_310)))
(let ((.def_12 (not b.counter.3__AT1)))
(let ((.def_312 (or .def_12 .def_311)))
(let ((.def_314 (and .def_312 .def_313)))
(let ((.def_320 (and .def_314 .def_319)))
(let ((.def_323 (and .def_320 .def_322)))
(let ((.def_85 (<= 0.0 b.delta__AT0)))
(let ((.def_49 (not b.EVENT.0__AT0)))
(let ((.def_47 (not b.EVENT.1__AT0)))
(let ((.def_71 (and .def_47 .def_49)))
(let ((.def_72 (not .def_71)))
(let ((.def_304 (or .def_72 .def_85)))
(let ((.def_305 (or b.EVENT.1__AT0 .def_304)))
(let ((.def_254 (= b.counter.0__AT1 b.counter.0__AT0)))
(let ((.def_252 (= b.counter.1__AT1 b.counter.1__AT0)))
(let ((.def_250 (= b.counter.2__AT1 b.counter.2__AT0)))
(let ((.def_249 (= b.counter.3__AT1 b.counter.3__AT0)))
(let ((.def_251 (and .def_249 .def_250)))
(let ((.def_253 (and .def_251 .def_252)))
(let ((.def_255 (and .def_253 .def_254)))
(let ((.def_301 (or .def_72 .def_255)))
(let ((.def_302 (or b.EVENT.1__AT0 .def_301)))
(let ((.def_261 (= b.x__AT1 b.x__AT0)))
(let ((.def_168 (= b.y__AT0 b.y__AT1)))
(let ((.def_295 (and .def_168 .def_261)))
(let ((.def_184 (= b.speed_x__AT0 b.speed_x__AT1)))
(let ((.def_296 (and .def_184 .def_295)))
(let ((.def_248 (= b.speed_y__AT0 b.speed_y__AT1)))
(let ((.def_297 (and .def_248 .def_296)))
(let ((.def_292 (= b.delta__AT0 0.0 )))
(let ((.def_293 (and .def_71 .def_292)))
(let ((.def_294 (not .def_293)))
(let ((.def_298 (or .def_294 .def_297)))
(let ((.def_299 (or b.EVENT.1__AT0 .def_298)))
(let ((.def_272 (= b.time__AT0 b.time__AT1)))
(let ((.def_284 (and .def_261 .def_272)))
(let ((.def_285 (and .def_168 .def_284)))
(let ((.def_286 (and .def_184 .def_285)))
(let ((.def_287 (and .def_248 .def_286)))
(let ((.def_288 (and .def_255 .def_287)))
(let ((.def_289 (or .def_47 .def_288)))
(let ((.def_275 (* (- 1.0) b.time__AT1)))
(let ((.def_278 (+ b.delta__AT0 .def_275)))
(let ((.def_279 (+ b.time__AT0 .def_278)))
(let ((.def_280 (= .def_279 0.0 )))
(let ((.def_281 (or .def_72 .def_280)))
(let ((.def_282 (or b.EVENT.1__AT0 .def_281)))
(let ((.def_273 (or .def_71 .def_272)))
(let ((.def_274 (or b.EVENT.1__AT0 .def_273)))
(let ((.def_283 (and .def_274 .def_282)))
(let ((.def_290 (and .def_283 .def_289)))
(let ((.def_268 (= .def_72 b.event_is_timed__AT1)))
(let ((.def_266 (= b.event_is_timed__AT0 .def_71)))
(let ((.def_269 (and .def_266 .def_268)))
(let ((.def_256 (and .def_248 .def_255)))
(let ((.def_208 (<= 0.0 b.speed_y__AT0)))
(let ((.def_209 (not .def_208)))
(let ((.def_207 (= b.y__AT0 0.0 )))
(let ((.def_210 (and .def_207 .def_209)))
(let ((.def_257 (or .def_210 .def_256)))
(let ((.def_225 (or .def_6 b.counter.0__AT0)))
(let ((.def_30 (not b.counter.0__AT0)))
(let ((.def_224 (or b.counter.0__AT1 .def_30)))
(let ((.def_226 (and .def_224 .def_225)))
(let ((.def_227 (and .def_4 .def_226)))
(let ((.def_228 (or b.counter.1__AT0 .def_227)))
(let ((.def_28 (not b.counter.1__AT0)))
(let ((.def_223 (or b.counter.1__AT1 .def_28)))
(let ((.def_229 (and .def_223 .def_228)))
(let ((.def_240 (and .def_9 .def_229)))
(let ((.def_241 (or b.counter.2__AT0 .def_240)))
(let ((.def_235 (and .def_4 .def_30)))
(let ((.def_236 (or b.counter.1__AT0 .def_235)))
(let ((.def_237 (and .def_223 .def_236)))
(let ((.def_238 (and b.counter.2__AT1 .def_237)))
(let ((.def_33 (not b.counter.2__AT0)))
(let ((.def_239 (or .def_33 .def_238)))
(let ((.def_242 (and .def_239 .def_241)))
(let ((.def_243 (and b.counter.3__AT1 .def_242)))
(let ((.def_244 (or b.counter.3__AT0 .def_243)))
(let ((.def_230 (and b.counter.2__AT1 .def_229)))
(let ((.def_231 (or b.counter.2__AT0 .def_230)))
(let ((.def_219 (or b.counter.1__AT1 b.counter.1__AT0)))
(let ((.def_217 (and .def_4 b.counter.0__AT1)))
(let ((.def_218 (or .def_28 .def_217)))
(let ((.def_220 (and .def_218 .def_219)))
(let ((.def_221 (and .def_9 .def_220)))
(let ((.def_222 (or .def_33 .def_221)))
(let ((.def_232 (and .def_222 .def_231)))
(let ((.def_233 (and .def_12 .def_232)))
(let ((.def_36 (not b.counter.3__AT0)))
(let ((.def_234 (or .def_36 .def_233)))
(let ((.def_245 (and .def_234 .def_244)))
(let ((.def_214 (* (- 1.0) b.speed_y__AT0)))
(let ((.def_212 (* (- 1.0) speed_loss__AT0)))
(let ((.def_213 (+ 1.0 .def_212)))
(let ((.def_215 (* .def_213 .def_214)))
(let ((.def_216 (= b.speed_y__AT1 .def_215)))
(let ((.def_246 (and .def_216 .def_245)))
(let ((.def_211 (not .def_210)))
(let ((.def_247 (or .def_211 .def_246)))
(let ((.def_258 (and .def_247 .def_257)))
(let ((.def_259 (and .def_168 .def_258)))
(let ((.def_260 (and .def_184 .def_259)))
(let ((.def_262 (and .def_260 .def_261)))
(let ((.def_263 (or .def_71 .def_262)))
(let ((.def_264 (or b.EVENT.1__AT0 .def_263)))
(let ((.def_195 (* b.delta__AT0 b.delta__AT0)))
(let ((.def_186 (* (- 1.0) g__AT0)))
(let ((.def_194 (* (/ 1 2) .def_186)))
(let ((.def_196 (* .def_194 .def_195)))
(let ((.def_198 (* (- 1.0) b.y__AT1)))
(let ((.def_201 (+ .def_198 .def_196)))
(let ((.def_104 (* b.speed_y__AT0 b.delta__AT0)))
(let ((.def_202 (+ .def_104 .def_201)))
(let ((.def_203 (+ b.y__AT0 .def_202)))
(let ((.def_204 (= .def_203 0.0 )))
(let ((.def_189 (* (- 1.0) b.speed_y__AT1)))
(let ((.def_187 (* b.delta__AT0 .def_186)))
(let ((.def_190 (+ .def_187 .def_189)))
(let ((.def_191 (+ b.speed_y__AT0 .def_190)))
(let ((.def_192 (= .def_191 0.0 )))
(let ((.def_176 (* (- 1.0) b.x__AT1)))
(let ((.def_180 (+ .def_176 b.x__AT0)))
(let ((.def_174 (* b.delta__AT0 b.speed_x__AT0)))
(let ((.def_181 (+ .def_174 .def_180)))
(let ((.def_182 (= .def_181 0.0 )))
(let ((.def_185 (and .def_182 .def_184)))
(let ((.def_193 (and .def_185 .def_192)))
(let ((.def_205 (and .def_193 .def_204)))
(let ((.def_206 (or .def_72 .def_205)))
(let ((.def_265 (and .def_206 .def_264)))
(let ((.def_270 (and .def_265 .def_269)))
(let ((.def_291 (and .def_270 .def_290)))
(let ((.def_300 (and .def_291 .def_299)))
(let ((.def_303 (and .def_300 .def_302)))
(let ((.def_306 (and .def_303 .def_305)))
(let ((.def_307 (and .def_47 .def_306)))
(let ((.def_324 (and .def_307 .def_323)))
(let ((.def_31 (and .def_28 .def_30)))
(let ((.def_34 (and .def_31 .def_33)))
(let ((.def_37 (and .def_34 .def_36)))
(let ((.def_325 (and .def_37 .def_324)))
(let ((.def_169 (not .def_168)))
(let ((.def_74 (* g__AT0 b.delta__AT0)))
(let ((.def_156 (<= b.speed_y__AT0 .def_74)))
(let ((.def_157 (not .def_156)))
(let ((.def_78 (* 2.0 b.speed_y__AT0)))
(let ((.def_79 (* b.delta__AT0 .def_78)))
(let ((.def_75 (* b.delta__AT0 .def_74)))
(let ((.def_77 (* (- 1.0) .def_75)))
(let ((.def_80 (+ .def_77 .def_79)))
(let ((.def_81 (* 2.0 b.y__AT0)))
(let ((.def_83 (+ .def_81 .def_80)))
(let ((.def_148 (= .def_83 0.0 )))
(let ((.def_149 (not .def_148)))
(let ((.def_158 (or .def_149 .def_157)))
(let ((.def_135 (* b.speed_y__AT0 .def_111)))
(let ((.def_136 (* b.delta__AT0 .def_135)))
(let ((.def_132 (* b.delta__AT0 .def_131)))
(let ((.def_133 (* b.delta__AT0 .def_132)))
(let ((.def_134 (* (- 1.0) .def_133)))
(let ((.def_137 (+ .def_134 .def_136)))
(let ((.def_112 (* b.y__AT0 .def_111)))
(let ((.def_138 (+ .def_112 .def_137)))
(let ((.def_143 (<= 0.0 .def_138)))
(let ((.def_88 (* b.speed_y__AT0 g__AT0)))
(let ((.def_140 (<= .def_132 .def_88)))
(let ((.def_144 (or .def_140 .def_143)))
(let ((.def_159 (and .def_144 .def_158)))
(let ((.def_90 (* b.speed_y__AT0 b.speed_y__AT0)))
(let ((.def_113 (+ .def_90 .def_112)))
(let ((.def_153 (<= .def_113 0.0 )))
(let ((.def_154 (not .def_153)))
(let ((.def_155 (or .def_128 .def_154)))
(let ((.def_160 (or .def_155 .def_159)))
(let ((.def_117 (* b.y__AT0 g__AT0)))
(let ((.def_123 (<= .def_117 0.0 )))
(let ((.def_122 (<= .def_88 0.0 )))
(let ((.def_124 (and .def_122 .def_123)))
(let ((.def_161 (or .def_124 .def_160)))
(let ((.def_146 (<= .def_74 b.speed_y__AT0)))
(let ((.def_147 (not .def_146)))
(let ((.def_150 (or .def_147 .def_149)))
(let ((.def_139 (<= .def_138 0.0 )))
(let ((.def_141 (and .def_139 .def_140)))
(let ((.def_142 (or .def_128 .def_141)))
(let ((.def_145 (and .def_142 .def_144)))
(let ((.def_151 (and .def_145 .def_150)))
(let ((.def_126 (= .def_113 0.0 )))
(let ((.def_129 (and .def_126 .def_128)))
(let ((.def_118 (<= 0.0 .def_117)))
(let ((.def_121 (and .def_118 .def_120)))
(let ((.def_125 (or .def_121 .def_124)))
(let ((.def_130 (or .def_125 .def_129)))
(let ((.def_152 (or .def_130 .def_151)))
(let ((.def_162 (and .def_152 .def_161)))
(let ((.def_114 (<= 0.0 .def_113)))
(let ((.def_115 (not .def_114)))
(let ((.def_116 (or .def_94 .def_115)))
(let ((.def_163 (or .def_116 .def_162)))
(let ((.def_105 (+ b.y__AT0 .def_104)))
(let ((.def_106 (= .def_105 0.0 )))
(let ((.def_107 (not .def_106)))
(let ((.def_101 (* b.delta__AT0 .def_90)))
(let ((.def_97 (* b.y__AT0 b.speed_y__AT0)))
(let ((.def_102 (+ .def_97 .def_101)))
(let ((.def_103 (<= .def_102 0.0 )))
(let ((.def_108 (and .def_103 .def_107)))
(let ((.def_98 (<= 0.0 .def_97)))
(let ((.def_91 (* b.speed_y__AT0 .def_90)))
(let ((.def_89 (* b.y__AT0 .def_88)))
(let ((.def_92 (+ .def_89 .def_91)))
(let ((.def_93 (<= .def_92 0.0 )))
(let ((.def_96 (or .def_93 .def_95)))
(let ((.def_99 (or .def_96 .def_98)))
(let ((.def_25 (= b.speed_y__AT0 0.0 )))
(let ((.def_100 (or .def_25 .def_99)))
(let ((.def_109 (or .def_100 .def_108)))
(let ((.def_86 (not .def_85)))
(let ((.def_84 (<= 0.0 .def_83)))
(let ((.def_87 (or .def_84 .def_86)))
(let ((.def_110 (and .def_87 .def_109)))
(let ((.def_164 (and .def_110 .def_163)))
(let ((.def_165 (and .def_71 .def_164)))
(let ((.def_166 (not .def_165)))
(let ((.def_170 (or .def_166 .def_169)))
(let ((.def_171 (or .def_72 .def_170)))
(let ((.def_66 (<= 0.0 b.y__AT0)))
(let ((.def_67 (and .def_65 .def_66)))
(let ((.def_50 (or .def_47 .def_49)))
(let ((.def_40 (or .def_28 .def_30)))
(let ((.def_44 (or b.counter.3__AT0 .def_40)))
(let ((.def_41 (or b.counter.2__AT0 .def_40)))
(let ((.def_39 (or .def_30 .def_33)))
(let ((.def_42 (and .def_39 .def_41)))
(let ((.def_43 (or .def_36 .def_42)))
(let ((.def_45 (and .def_43 .def_44)))
(let ((.def_51 (and .def_45 .def_50)))
(let ((.def_68 (and .def_51 .def_67)))
(let ((.def_22 (= b.y__AT0 10.0 )))
(let ((.def_17 (= b.time__AT0 0.0 )))
(let ((.def_19 (and .def_17 b.event_is_timed__AT0)))
(let ((.def_23 (and .def_19 .def_22)))
(let ((.def_26 (and .def_23 .def_25)))
(let ((.def_38 (and .def_26 .def_37)))
(let ((.def_69 (and .def_38 .def_68)))
(let ((.def_7 (and .def_4 .def_6)))
(let ((.def_10 (and .def_7 .def_9)))
(let ((.def_13 (and .def_10 .def_12)))
(let ((.def_14 (not .def_13)))
(let ((.def_70 (and .def_14 .def_69)))
(let ((.def_172 (and .def_70 .def_171)))
(let ((.def_326 (and .def_172 .def_325)))
(let ((.def_420 (and .def_326 .def_419)))
.def_420)))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))
(check-sat)
(exit)
