(set-logic QF_NRA)
(set-info :source |Benchmarks generated from hycomp (https://es-static.fbk.eu/tools/hycomp/). BMC instances of non-linear hybrid automata taken from: Alessandro Cimatti, Sergio Mover, Stefano Tonetta, A quantifier-free SMT encoding of non-linear hybrid automata, FMCAD 2012 and Alessandro Cimatti, Sergio Mover, Stefano Tonetta, Quantier-free encoding of invariants for Hybrid Systems, Formal Methods in System Design. This instance solves a BMC problem of depth 1 and uses the quantifier free encoding with lemmas encoding. Contacts: Sergio Mover (mover@fbk.eu), Stefano Tonetta (tonettas@fbk.eu), Alessandro Cimatti (cimatti@fbk.eu).|)
(set-info :smt-lib-version 2.0)
(set-info :category "industrial")
(set-info :status unsat)
;; MathSAT API call trace
;; generated on Mon Mar 19 10:42:15 2012
(declare-fun b.counter.3__AT1 () Bool)
(declare-fun b.delta__AT1 () Real)
(declare-fun b.EVENT.0__AT1 () Bool)
(declare-fun b.x__AT1 () Real)
(declare-fun b.EVENT.1__AT1 () Bool)
(declare-fun b.event_is_timed__AT0 () Bool)
(declare-fun b.y__AT0 () Real)
(declare-fun b.counter.0__AT0 () Bool)
(declare-fun b.counter.1__AT0 () Bool)
(declare-fun b.EVENT.0__AT0 () Bool)
(declare-fun b.bool_atom__AT0 () Bool)
(declare-fun b.counter.2__AT0 () Bool)
(declare-fun b.speed_y__AT1 () Real)
(declare-fun b.event_is_timed__AT1 () Bool)
(declare-fun b.x__AT0 () Real)
(declare-fun b.time__AT1 () Real)
(declare-fun b.bool_atom__AT1 () Bool)
(declare-fun b.g__AT0 () Real)
(declare-fun b.EVENT.1__AT0 () Bool)
(declare-fun b.time__AT0 () Real)
(declare-fun b.delta__AT0 () Real)
(declare-fun b.counter.3__AT0 () Bool)
(declare-fun b.speed_y__AT0 () Real)
(declare-fun b.y__AT1 () Real)
(declare-fun b.counter.0__AT1 () Bool)
(declare-fun b.counter.1__AT1 () Bool)
(declare-fun b.counter.2__AT1 () Bool)
(assert (let ((.def_66 (* (- 1.0) b.g__AT0)))
(let ((.def_69 (* (/ 1 2) .def_66)))
(let ((.def_81 (* 2.0 .def_69)))
(let ((.def_125 (* (- 1.0) .def_81)))
(let ((.def_353 (* .def_125 b.delta__AT1)))
(let ((.def_356 (<= .def_353 b.speed_y__AT1)))
(let ((.def_363 (not .def_356)))
(let ((.def_316 (<= 0.0 b.speed_y__AT1)))
(let ((.def_364 (or .def_316 .def_363)))
(let ((.def_354 (<= b.speed_y__AT1 .def_353)))
(let ((.def_330 (<= b.speed_y__AT1 0.0 )))
(let ((.def_331 (not .def_330)))
(let ((.def_362 (or .def_331 .def_354)))
(let ((.def_365 (and .def_362 .def_364)))
(let ((.def_116 (<= b.g__AT0 0.0 )))
(let ((.def_366 (or .def_116 .def_365)))
(let ((.def_358 (not .def_354)))
(let ((.def_359 (or .def_330 .def_358)))
(let ((.def_317 (not .def_316)))
(let ((.def_357 (or .def_317 .def_356)))
(let ((.def_360 (and .def_357 .def_359)))
(let ((.def_121 (<= 0.0 b.g__AT0)))
(let ((.def_361 (or .def_121 .def_360)))
(let ((.def_367 (and .def_361 .def_366)))
(let ((.def_355 (and .def_330 .def_354)))
(let ((.def_368 (and .def_355 .def_367)))
(let ((.def_312 (* .def_81 b.delta__AT1)))
(let ((.def_313 (+ b.speed_y__AT1 .def_312)))
(let ((.def_328 (<= .def_313 0.0 )))
(let ((.def_329 (not .def_328)))
(let ((.def_348 (or .def_329 .def_330)))
(let ((.def_314 (<= 0.0 .def_313)))
(let ((.def_347 (or .def_314 .def_317)))
(let ((.def_349 (and .def_347 .def_348)))
(let ((.def_350 (or .def_121 .def_349)))
(let ((.def_315 (not .def_314)))
(let ((.def_344 (or .def_315 .def_316)))
(let ((.def_343 (or .def_328 .def_331)))
(let ((.def_345 (and .def_343 .def_344)))
(let ((.def_346 (or .def_116 .def_345)))
(let ((.def_351 (and .def_346 .def_350)))
(let ((.def_342 (and .def_314 .def_316)))
(let ((.def_352 (and .def_342 .def_351)))
(let ((.def_369 (or .def_352 .def_368)))
(let ((.def_303 (* b.speed_y__AT1 b.delta__AT1)))
(let ((.def_301 (* b.delta__AT1 b.delta__AT1)))
(let ((.def_302 (* .def_69 .def_301)))
(let ((.def_304 (+ .def_302 .def_303)))
(let ((.def_289 (* (- 1.0) b.x__AT1)))
(let ((.def_290 (* b.x__AT1 .def_289)))
(let ((.def_305 (* (- 1.0) .def_290)))
(let ((.def_308 (+ .def_305 .def_304)))
(let ((.def_309 (+ b.y__AT1 .def_308)))
(let ((.def_322 (<= .def_309 0.0 )))
(let ((.def_336 (not .def_322)))
(let ((.def_320 (<= b.y__AT1 .def_290)))
(let ((.def_337 (or .def_320 .def_336)))
(let ((.def_291 (<= .def_290 b.y__AT1)))
(let ((.def_334 (not .def_291)))
(let ((.def_310 (<= 0.0 .def_309)))
(let ((.def_335 (or .def_310 .def_334)))
(let ((.def_338 (and .def_335 .def_337)))
(let ((.def_332 (or .def_329 .def_331)))
(let ((.def_333 (not .def_332)))
(let ((.def_339 (or .def_333 .def_338)))
(let ((.def_324 (not .def_310)))
(let ((.def_325 (or .def_291 .def_324)))
(let ((.def_321 (not .def_320)))
(let ((.def_323 (or .def_321 .def_322)))
(let ((.def_326 (and .def_323 .def_325)))
(let ((.def_318 (or .def_315 .def_317)))
(let ((.def_319 (not .def_318)))
(let ((.def_327 (or .def_319 .def_326)))
(let ((.def_340 (and .def_327 .def_339)))
(let ((.def_311 (and .def_291 .def_310)))
(let ((.def_341 (and .def_311 .def_340)))
(let ((.def_370 (and .def_341 .def_369)))
(let ((.def_58 (<= b.g__AT0 10.0 )))
(let ((.def_57 (<= 8.0 b.g__AT0)))
(let ((.def_59 (and .def_57 .def_58)))
(let ((.def_371 (and .def_59 .def_370)))
(let ((.def_28 (not b.counter.0__AT0)))
(let ((.def_295 (or b.counter.1__AT0 .def_28)))
(let ((.def_31 (not b.counter.2__AT0)))
(let ((.def_296 (or .def_31 .def_295)))
(let ((.def_297 (or b.counter.3__AT0 .def_296)))
(let ((.def_292 (and .def_59 .def_291)))
(let ((.def_286 (not b.EVENT.0__AT1)))
(let ((.def_284 (not b.EVENT.1__AT1)))
(let ((.def_287 (or .def_284 .def_286)))
(let ((.def_199 (not b.counter.1__AT1)))
(let ((.def_4 (not b.counter.0__AT1)))
(let ((.def_277 (or .def_4 .def_199)))
(let ((.def_281 (or b.counter.3__AT1 .def_277)))
(let ((.def_278 (or b.counter.2__AT1 .def_277)))
(let ((.def_8 (not b.counter.2__AT1)))
(let ((.def_276 (or .def_4 .def_8)))
(let ((.def_279 (and .def_276 .def_278)))
(let ((.def_216 (not b.counter.3__AT1)))
(let ((.def_280 (or .def_216 .def_279)))
(let ((.def_282 (and .def_280 .def_281)))
(let ((.def_288 (and .def_282 .def_287)))
(let ((.def_293 (and .def_288 .def_292)))
(let ((.def_271 (<= 0.0 b.delta__AT0)))
(let ((.def_47 (not b.EVENT.0__AT0)))
(let ((.def_45 (not b.EVENT.1__AT0)))
(let ((.def_146 (and .def_45 .def_47)))
(let ((.def_148 (not .def_146)))
(let ((.def_272 (or .def_148 .def_271)))
(let ((.def_273 (or b.EVENT.1__AT0 .def_272)))
(let ((.def_184 (= b.bool_atom__AT0 b.bool_atom__AT1)))
(let ((.def_179 (= b.counter.0__AT1 b.counter.0__AT0)))
(let ((.def_177 (= b.counter.1__AT1 b.counter.1__AT0)))
(let ((.def_175 (= b.counter.2__AT1 b.counter.2__AT0)))
(let ((.def_174 (= b.counter.3__AT1 b.counter.3__AT0)))
(let ((.def_176 (and .def_174 .def_175)))
(let ((.def_178 (and .def_176 .def_177)))
(let ((.def_180 (and .def_178 .def_179)))
(let ((.def_267 (and .def_180 .def_184)))
(let ((.def_268 (or .def_148 .def_267)))
(let ((.def_269 (or b.EVENT.1__AT0 .def_268)))
(let ((.def_256 (* b.delta__AT0 .def_66)))
(let ((.def_257 (* (- 1.0) b.speed_y__AT1)))
(let ((.def_260 (+ .def_257 .def_256)))
(let ((.def_261 (+ b.speed_y__AT0 .def_260)))
(let ((.def_262 (= .def_261 0.0 )))
(let ((.def_248 (* (- 1.0) b.y__AT1)))
(let ((.def_72 (* b.delta__AT0 b.speed_y__AT0)))
(let ((.def_251 (+ .def_72 .def_248)))
(let ((.def_65 (* b.delta__AT0 b.delta__AT0)))
(let ((.def_70 (* .def_65 .def_69)))
(let ((.def_252 (+ .def_70 .def_251)))
(let ((.def_253 (+ b.y__AT0 .def_252)))
(let ((.def_254 (= .def_253 0.0 )))
(let ((.def_166 (= b.x__AT0 b.x__AT1)))
(let ((.def_255 (and .def_166 .def_254)))
(let ((.def_263 (and .def_255 .def_262)))
(let ((.def_264 (or .def_148 .def_263)))
(let ((.def_169 (= b.y__AT0 b.y__AT1)))
(let ((.def_243 (and .def_166 .def_169)))
(let ((.def_172 (= b.speed_y__AT0 b.speed_y__AT1)))
(let ((.def_244 (and .def_172 .def_243)))
(let ((.def_240 (= b.delta__AT0 0.0 )))
(let ((.def_241 (and .def_146 .def_240)))
(let ((.def_242 (not .def_241)))
(let ((.def_245 (or .def_242 .def_244)))
(let ((.def_246 (or b.EVENT.1__AT0 .def_245)))
(let ((.def_233 (and .def_172 .def_180)))
(let ((.def_234 (or b.bool_atom__AT0 .def_233)))
(let ((.def_208 (or .def_4 b.counter.0__AT0)))
(let ((.def_207 (or b.counter.0__AT1 .def_28)))
(let ((.def_209 (and .def_207 .def_208)))
(let ((.def_210 (and .def_199 .def_209)))
(let ((.def_211 (or b.counter.1__AT0 .def_210)))
(let ((.def_26 (not b.counter.1__AT0)))
(let ((.def_206 (or b.counter.1__AT1 .def_26)))
(let ((.def_212 (and .def_206 .def_211)))
(let ((.def_224 (and .def_8 .def_212)))
(let ((.def_225 (or b.counter.2__AT0 .def_224)))
(let ((.def_219 (and .def_28 .def_199)))
(let ((.def_220 (or b.counter.1__AT0 .def_219)))
(let ((.def_221 (and .def_206 .def_220)))
(let ((.def_222 (and b.counter.2__AT1 .def_221)))
(let ((.def_223 (or .def_31 .def_222)))
(let ((.def_226 (and .def_223 .def_225)))
(let ((.def_227 (and b.counter.3__AT1 .def_226)))
(let ((.def_228 (or b.counter.3__AT0 .def_227)))
(let ((.def_213 (and b.counter.2__AT1 .def_212)))
(let ((.def_214 (or b.counter.2__AT0 .def_213)))
(let ((.def_202 (or b.counter.1__AT1 b.counter.1__AT0)))
(let ((.def_200 (and b.counter.0__AT1 .def_199)))
(let ((.def_201 (or .def_26 .def_200)))
(let ((.def_203 (and .def_201 .def_202)))
(let ((.def_204 (and .def_8 .def_203)))
(let ((.def_205 (or .def_31 .def_204)))
(let ((.def_215 (and .def_205 .def_214)))
(let ((.def_217 (and .def_215 .def_216)))
(let ((.def_34 (not b.counter.3__AT0)))
(let ((.def_218 (or .def_34 .def_217)))
(let ((.def_229 (and .def_218 .def_228)))
(let ((.def_196 (* b.speed_y__AT0 b.speed_y__AT0)))
(let ((.def_195 (* b.speed_y__AT1 b.speed_y__AT1)))
(let ((.def_197 (= .def_195 .def_196)))
(let ((.def_193 (+ b.speed_y__AT0 b.speed_y__AT1)))
(let ((.def_194 (= .def_193 0.0 )))
(let ((.def_198 (and .def_194 .def_197)))
(let ((.def_230 (and .def_198 .def_229)))
(let ((.def_192 (not b.bool_atom__AT0)))
(let ((.def_231 (or .def_192 .def_230)))
(let ((.def_52 (* (- 1.0) b.x__AT0)))
(let ((.def_53 (* b.x__AT0 .def_52)))
(let ((.def_189 (= b.y__AT0 .def_53)))
(let ((.def_86 (<= 0.0 b.speed_y__AT0)))
(let ((.def_87 (not .def_86)))
(let ((.def_190 (and .def_87 .def_189)))
(let ((.def_191 (= b.bool_atom__AT0 .def_190)))
(let ((.def_232 (and .def_191 .def_231)))
(let ((.def_235 (and .def_232 .def_234)))
(let ((.def_236 (and .def_166 .def_235)))
(let ((.def_237 (and .def_169 .def_236)))
(let ((.def_238 (or .def_146 .def_237)))
(let ((.def_239 (or b.EVENT.1__AT0 .def_238)))
(let ((.def_247 (and .def_239 .def_246)))
(let ((.def_265 (and .def_247 .def_264)))
(let ((.def_153 (= b.time__AT0 b.time__AT1)))
(let ((.def_167 (and .def_153 .def_166)))
(let ((.def_170 (and .def_167 .def_169)))
(let ((.def_173 (and .def_170 .def_172)))
(let ((.def_181 (and .def_173 .def_180)))
(let ((.def_185 (and .def_181 .def_184)))
(let ((.def_186 (or .def_45 .def_185)))
(let ((.def_156 (* (- 1.0) b.time__AT1)))
(let ((.def_159 (+ b.delta__AT0 .def_156)))
(let ((.def_160 (+ b.time__AT0 .def_159)))
(let ((.def_161 (= .def_160 0.0 )))
(let ((.def_162 (or .def_148 .def_161)))
(let ((.def_163 (or b.EVENT.1__AT0 .def_162)))
(let ((.def_154 (or .def_146 .def_153)))
(let ((.def_155 (or b.EVENT.1__AT0 .def_154)))
(let ((.def_164 (and .def_155 .def_163)))
(let ((.def_187 (and .def_164 .def_186)))
(let ((.def_150 (= .def_148 b.event_is_timed__AT1)))
(let ((.def_147 (= b.event_is_timed__AT0 .def_146)))
(let ((.def_151 (and .def_147 .def_150)))
(let ((.def_188 (and .def_151 .def_187)))
(let ((.def_266 (and .def_188 .def_265)))
(let ((.def_270 (and .def_266 .def_269)))
(let ((.def_274 (and .def_270 .def_273)))
(let ((.def_275 (and .def_45 .def_274)))
(let ((.def_294 (and .def_275 .def_293)))
(let ((.def_298 (and .def_294 .def_297)))
(let ((.def_126 (* b.delta__AT0 .def_125)))
(let ((.def_129 (<= .def_126 b.speed_y__AT0)))
(let ((.def_136 (not .def_129)))
(let ((.def_137 (or .def_86 .def_136)))
(let ((.def_127 (<= b.speed_y__AT0 .def_126)))
(let ((.def_100 (<= b.speed_y__AT0 0.0 )))
(let ((.def_101 (not .def_100)))
(let ((.def_135 (or .def_101 .def_127)))
(let ((.def_138 (and .def_135 .def_137)))
(let ((.def_139 (or .def_116 .def_138)))
(let ((.def_131 (not .def_127)))
(let ((.def_132 (or .def_100 .def_131)))
(let ((.def_130 (or .def_87 .def_129)))
(let ((.def_133 (and .def_130 .def_132)))
(let ((.def_134 (or .def_121 .def_133)))
(let ((.def_140 (and .def_134 .def_139)))
(let ((.def_128 (and .def_100 .def_127)))
(let ((.def_141 (and .def_128 .def_140)))
(let ((.def_82 (* b.delta__AT0 .def_81)))
(let ((.def_83 (+ b.speed_y__AT0 .def_82)))
(let ((.def_98 (<= .def_83 0.0 )))
(let ((.def_99 (not .def_98)))
(let ((.def_119 (or .def_99 .def_100)))
(let ((.def_84 (<= 0.0 .def_83)))
(let ((.def_118 (or .def_84 .def_87)))
(let ((.def_120 (and .def_118 .def_119)))
(let ((.def_122 (or .def_120 .def_121)))
(let ((.def_85 (not .def_84)))
(let ((.def_114 (or .def_85 .def_86)))
(let ((.def_113 (or .def_98 .def_101)))
(let ((.def_115 (and .def_113 .def_114)))
(let ((.def_117 (or .def_115 .def_116)))
(let ((.def_123 (and .def_117 .def_122)))
(let ((.def_112 (and .def_84 .def_86)))
(let ((.def_124 (and .def_112 .def_123)))
(let ((.def_142 (or .def_124 .def_141)))
(let ((.def_73 (+ .def_70 .def_72)))
(let ((.def_74 (* (- 1.0) .def_53)))
(let ((.def_77 (+ .def_74 .def_73)))
(let ((.def_78 (+ b.y__AT0 .def_77)))
(let ((.def_92 (<= .def_78 0.0 )))
(let ((.def_106 (not .def_92)))
(let ((.def_90 (<= b.y__AT0 .def_53)))
(let ((.def_107 (or .def_90 .def_106)))
(let ((.def_54 (<= .def_53 b.y__AT0)))
(let ((.def_104 (not .def_54)))
(let ((.def_79 (<= 0.0 .def_78)))
(let ((.def_105 (or .def_79 .def_104)))
(let ((.def_108 (and .def_105 .def_107)))
(let ((.def_102 (or .def_99 .def_101)))
(let ((.def_103 (not .def_102)))
(let ((.def_109 (or .def_103 .def_108)))
(let ((.def_94 (not .def_79)))
(let ((.def_95 (or .def_54 .def_94)))
(let ((.def_91 (not .def_90)))
(let ((.def_93 (or .def_91 .def_92)))
(let ((.def_96 (and .def_93 .def_95)))
(let ((.def_88 (or .def_85 .def_87)))
(let ((.def_89 (not .def_88)))
(let ((.def_97 (or .def_89 .def_96)))
(let ((.def_110 (and .def_97 .def_109)))
(let ((.def_80 (and .def_54 .def_79)))
(let ((.def_111 (and .def_80 .def_110)))
(let ((.def_143 (and .def_111 .def_142)))
(let ((.def_144 (and .def_59 .def_143)))
(let ((.def_60 (and .def_54 .def_59)))
(let ((.def_48 (or .def_45 .def_47)))
(let ((.def_38 (or .def_26 .def_28)))
(let ((.def_42 (or b.counter.3__AT0 .def_38)))
(let ((.def_39 (or b.counter.2__AT0 .def_38)))
(let ((.def_37 (or .def_28 .def_31)))
(let ((.def_40 (and .def_37 .def_39)))
(let ((.def_41 (or .def_34 .def_40)))
(let ((.def_43 (and .def_41 .def_42)))
(let ((.def_49 (and .def_43 .def_48)))
(let ((.def_61 (and .def_49 .def_60)))
(let ((.def_29 (and .def_26 .def_28)))
(let ((.def_32 (and .def_29 .def_31)))
(let ((.def_35 (and .def_32 .def_34)))
(let ((.def_23 (= b.y__AT0 10.0 )))
(let ((.def_19 (= b.x__AT0 0.0 )))
(let ((.def_15 (= b.time__AT0 0.0 )))
(let ((.def_17 (and .def_15 b.event_is_timed__AT0)))
(let ((.def_20 (and .def_17 .def_19)))
(let ((.def_24 (and .def_20 .def_23)))
(let ((.def_36 (and .def_24 .def_35)))
(let ((.def_62 (and .def_36 .def_61)))
(let ((.def_6 (or .def_4 b.counter.1__AT1)))
(let ((.def_9 (or .def_6 .def_8)))
(let ((.def_11 (or .def_9 b.counter.3__AT1)))
(let ((.def_12 (not .def_11)))
(let ((.def_63 (and .def_12 .def_62)))
(let ((.def_145 (and .def_63 .def_144)))
(let ((.def_299 (and .def_145 .def_298)))
(let ((.def_372 (and .def_299 .def_371)))
.def_372)))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))
(check-sat)
(exit)
